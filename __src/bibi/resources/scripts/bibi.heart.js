...
Bibi.verifySettingValue = (SettingType, _P, _V, Fill) =>
  Bibi.verifySettingValue[SettingType](_P, _V, Fill);
((Verifiers) => {
  for (const SettingType in Verifiers)
    Bibi.verifySettingValue[SettingType] = Verifiers[SettingType];
})({
  boolean: (_P, _V, Fill) => {
  ...
  string: (_P, _V, Fill) => {
    if (typeof _V == "string") {
      switch (_P) {
        case "edge":
          return /^(head|foot)$/.test(_V) ? _V : undefined;
        case "book":
          return (_V = decodeURIComponent(_V).trim()) ? _V : undefined;
        case "default-page-progression-direction":
          return _V == "rtl" ? _V : "ltr";
        case "on-doubletap":
        case "on-tripletap":
          return /^(panel|zoom)$/.test(_V) ? _V : undefined;
        case "p":
          return /^([a-z]+|[1-9]\d*((\.[1-9]\d*)*|-[a-z]+))$/.test(_V)
            ? _V
            : undefined;
        case "pagination-method":
          return _V == "x" ? _V : "auto";
        case "reader-view-mode":
          return /^(paged|horizontal|vertical)$/.test(_V) ? _V : "paged";
      }
      return _V;
    }
    if (Fill) return "";
  ...
  integer: (_P, _V, Fill) => {
    if (typeof (_V *= 1) == "number" && isFinite(_V)) {
      ...
      switch (_P) {
        case "log":
          return Math.min(_V, 9);
        case "max-bookmarks":
          return Math.min(_V, 9);
        case "max-history":
          return Math.min(_V, 19);
      }
      return _V;
    }
    if (Fill) return 0;
    ...
    if (Array.isArray(_V)) {
      switch (_P) {
        ...
        case "extensions":
          return _V.filter(
            (_I) =>
              typeof _I["src"] == "string" && (_I["src"] = _I["src"].trim())
          );
        case "extract-if-necessary":
          return (_V = _V.map((_I) =>
            typeof _I == "string" ? _I.trim().toLowerCase() : ""
          )).includes("*")
            ? ["*"]
            : _V.filter((_I) => /^(\.[\w\d]+)*$/.test(_I));
        ...
        case "orthogonal-wheelings":
          _V.length = 2;
          for (let i = 0; i < 2; i++)
            _V[i] = typeof _V[i] == "string" ? _V[i] : "";
          return _V;
        case "trustworthy-origins":
          return _V.reduce(
            (_VN, _I) =>
              typeof _I == "string" &&
              /^https?:\/\/[^\/]+$/.test((_I = _I.trim().replace(/\/$/, ""))) &&
              !_VN.includes(_I)
                ? _VN.push(_I) && _VN
                : false,
            []
    ...
    for (const ST in STs) {
      STs[ST].forEach((_P) => {
        ...
        if (Fill) {
          ...
          if (typeof VSV != "undefined" || typeof To[_P] == "undefined")
            To[_P] = Bibi.verifySettingValue[ST](_P, From[_P], true);
...
(Bibi.at1st = () =>
  Bibi.at1st.List.forEach((fn) => (typeof fn == "function" ? fn() : true))),
  (Bibi.at1st.List = []);
...
Bibi.initialize = () => {
  ...
  }
  {
    ...
    if ((O.TouchOS = sML.OS.iOS || sML.OS.Android ? true : false)) {
      // Touch Device
    ...
    O.HTML.classList.add(
      "default-lang-" +
        (O.Language = ((NLs) => {
          // Language
          if (Array.isArray(navigator.languages))
            NLs = NLs.concat(navigator.languages);
          if (navigator.language && navigator.language != NLs[0])
            NLs.unshift(navigator.language);
  ...
  }
  {
    ...
    O.Embedded = (() => {
      // Window Embedded or Not
      if (window.parent == window) {
        O.HTML.classList.add("window-direct");
        return 0;
      } // false
      else {
        ...
        return -1;
      } // true (1:Reachable or -1:Unreachable)
    })();
    O.ParentBibi =
      O.Embedded === 1 && typeof S["parent-bibi-index"] == "number"
        ? window.parent["bibi:jo"].Bibis[S["parent-bibi-index"]] || null
        : null;
    ...
    O.FullscreenTarget = (() => {
      ...
      const FsT =
        (() => {
          if (!O.Embedded) {
            sML.Fullscreen.polyfill(window);
            return O.HTML;
          }
          if (O.ParentBibi) {
            sML.Fullscreen.polyfill(window.parent);
            return O.ParentBibi.Frame;
          }
        })() || null;
      if (FsT && FsT.ownerDocument.fullscreenEnabled) {
        O.HTML.classList.add("fullscreen-enabled");
        return FsT;
      } else {
        O.HTML.classList.add("fullscreen-disabled");
        return null;
      }
    })();
    if (O.ParentBibi) {
      (O.ParentBibi.Window = window),
        (O.ParentBibi.Document = document),
        (O.ParentBibi.HTML = O.HTML),
        (O.ParentBibi.Body = O.Body);
      [
        "bibi:initialized",
        "bibi:readied",
        "bibi:prepared",
        "bibi:opened",
      ].forEach((EN) => E.add(EN, (Det) => O.ParentBibi.dispatch(EN, Det)));
  ...
  }
  {
    ...
    O.WritingModeProperty = (() => {
      ...
      if (
        /^(vertical|horizontal)-/.test(HTMLComputedStyle["writing-mode"]) ||
        sML.UA.Trident
      )
        return "writing-mode";
      if (
        /^(vertical|horizontal)-/.test(
          HTMLComputedStyle["-webkit-writing-mode"]
        )
      )
        return "-webkit-writing-mode";
      if (
        /^(vertical|horizontal)-/.test(HTMLComputedStyle["-epub-writing-mode"])
      )
        return "-epub-writing-mode";
      return undefined;
    })();
    const StyleChecker = O.Body.appendChild(
      sML.create("div", {
    ...
    O.VerticalTextEnabled =
      StyleChecker.offsetWidth < StyleChecker.offsetHeight;
    O.DefaultFontSize = Math.min(
      StyleChecker.offsetWidth,
      StyleChecker.offsetHeight
    );
    StyleChecker.style.fontSize = "0.01px";
    O.MinimumFontSize = Math.min(
      StyleChecker.offsetWidth,
      StyleChecker.offsetHeight
    );
  ...
  E.dispatch(
    "bibi:initialized",
    (Bibi.Status = Bibi.Initialized = "Initialized")
  );
...
Bibi.loadExtensions = () => {
  return new Promise((resolve, reject) => {
    ...
    if (!S["allow-scripts-in-content"])
      AdditionalExtensions.push("sanitizer.js");
    let ReadyForExtraction = false,
      ReadyForBibiZine = false;
    if (S["book"]) {
      if (O.isToBeExtractedIfNecessary(S["book"])) ReadyForExtraction = true;
      if (S["zine"]) ReadyForBibiZine = true;
    } else {
      if (S["accept-local-file"] || S["accept-blob-converted-data"])
        ReadyForExtraction = ReadyForBibiZine = true;
    }
    if (ReadyForBibiZine) AdditionalExtensions.unshift("zine.js");
    (ReadyForExtraction
      ? (S["book"]
          ? O.tryRangeRequest().then(() => "on-the-fly")
          : Promise.reject()
        )
          .catch(() => "at-once")
          .then((_) => AdditionalExtensions.unshift("extractor/" + _ + ".js"))
      : Promise.resolve()
    ).then(() => {
      if (AdditionalExtensions.length)
        AdditionalExtensions.forEach((AX) =>
          S["extensions"].unshift({
            src: new URL("../../extensions/" + AX, Bibi.Script.src).href,
          })
        );
      if (S["extensions"].length == 0) return reject();
      O.log(
        `Loading Extension${S["extensions"].length > 1 ? "s" : ""}...`,
        "<g:>"
      );
      const loadExtensionInPreset = (i) => {
        X.load(S["extensions"][i])
          .then((Msg) => {
            //O.log(Msg);
          })
          .catch((Msg) => {
    ...
    .then(() => {
      ...
      O.log(
        X.Extensions.length
          ? `Loaded. (${X.Extensions.length} Extension${
              X.Extensions.length > 1 ? "s" : ""
            })`
          : `No Extension.`,
        "</g>"
      );
...
Bibi.ready = () =>
  new Promise((resolve) => {
    ...
    setTimeout(
      () =>
        E.dispatch("bibi:readied", (Bibi.Status = Bibi.Readied = "Readied")),
      O.TouchOS && !O.Embedded ? 1234 : 0
    );
  }).then(() => {
...
Bibi.getBookData = () =>
  S["book-data"]
    ? Promise.resolve({
        BookData: S["book-data"],
        BookDataMIMEType: S["book-data-mimetype"],
      })
    : S["book"]
    ? Promise.resolve({ Book: S["book"] })
    : S["accept-local-file"]
    ? new Promise((resolve) => {
        Bibi.getBookData.resolve = (Par) => {
          resolve(Par), O.HTML.classList.remove("waiting-file");
        };
        O.HTML.classList.add("waiting-file");
      })
    : Promise.reject(
        `Tell me EPUB name via ${O.Embedded ? "embedding tag" : "URI"}.`
      );

Bibi.setBookData = (Par) =>
  Bibi.getBookData.resolve
    ? Bibi.getBookData.resolve(Par)
    : Promise.reject(Par);
  ...
  Promise.resolve()
    .then(() => {
      ...
      return L.initializeBook(BookInfo).then((InitializedAs) => {
        ...
        O.log(
          `Initialized. (as ${
            /^[aiueo]/i.test(InitializedAs) ? "an" : "a"
          } ${InitializedAs})`,
          "</g>"
        );
      });
    ...
    .then(() => {
      ...
      if (typeof R.StartOn == "object") {
            ...
            : (() => {
                ...
                if (typeof R.StartOn.IIPP == "number") {
                  ...
                  if (II < 0) (II = 0), (R.StartOn = { IIPP: II });
                    ...
                    (II = R.Items.length - 1), (R.StartOn = { IIPP: II });
                  return R.Items[II];
                }
        ...
        LayoutOption.TargetSpreadIndex =
          Item && Item.Spread ? Item.Spread.Index : 0;
          ...
          new Promise((resolve) =>
            L.loadSpread(Spread, {
              AllowPlaceholderItems:
                S["allow-placeholders"] &&
                Spread.Index != LayoutOption.TargetSpreadIndex,
            }).then(() => {
              LoadedItems += Spread.Items.length;
              I.note(
                `Loading... (${LoadedItems}/${R.Items.length} Items Loaded.)`
              );
              !LayoutOption.Reset
                ? R.layOutSpreadAndItsItems(Spread).then(resolve)
                : resolve();
            })
          )
        )
  ...
  new Promise((resolve) => {
    ...
    if (!I.History.List.length) {
      I.History.List = [
        {
          UI: Bibi,
          Item: LandingPage.Item,
          PageProgressInItem:
            LandingPage.IndexInItem / LandingPage.Item.Pages.length,
        },
      ];
      I.History.update();
    }
    if (S["allow-placeholders"])
      E.add("bibi:scrolled", () => I.PageObserver.turnItems());
    if (S["resume-from-last-position"])
      E.add("bibi:changed-intersection", () => {
        try {
          if (O.Biscuits)
            O.Biscuits.memorize("Book", {
              Position: { IIPP: I.PageObserver.getIIPP() },
            });
        } catch (Err) {}
      });
    ...
    Bibi.Dev && location.hostname != "localhost"
      ? Bibi.createDevNote()
...
Bibi.createDevNote = () => {
  const Dev = (Bibi.IsDevMode = O.Body.appendChild(
    sML.create("div", { id: "bibi-is-dev-mode" })
  ));
  Bibi.createDevNote.logBorderLine();
  Bibi.createDevNote.appendParagraph(
  ...
  Bibi.createDevNote
    ...
    .addEventListener("click", () => (Dev.className = "hide"));
  Bibi.createDevNote.logBorderLine();
  [
    E["pointerdown"],
    E["pointerup"],
...
Bibi.createDevNote.appendParagraph = (InnerHTML, NoLog) => {
  const P = Bibi.IsDevMode.appendChild(
    sML.create("p", { innerHTML: InnerHTML })
  );
...
Bibi.createElement = (...Args) => {
  ...
  if (window.customElements) {
    if (!Bibi.Elements[TagName])
      (Bibi.Elements[TagName] = class extends HTMLElement {
        constructor() {
          super();
        }
      }),
        window.customElements.define(TagName, Bibi.Elements[TagName]);
  } else if (document.registerElement) {
    if (!Bibi.Elements[TagName])
      Bibi.Elements[TagName] = document.registerElement(TagName);
...
L.initializeBook = (BookInfo = {}) =>
  new Promise((resolve, reject) => {
    const reject_failedToOpenTheBook = (Msg) =>
      reject(`Failed to open the book (${Msg})`);
    if (!BookInfo.Book && !BookInfo.BookData)
      return reject_failedToOpenTheBook(Bibi.ErrorMessages.DataInvalid);
    const BookDataFormat =
      typeof BookInfo.Book == "string"
        ? "URI"
        : typeof BookInfo.BookData == "string"
        ? "Base64"
        : typeof BookInfo.BookData == "object" &&
          BookInfo.BookData.size &&
          BookInfo.BookData.type
        ? BookInfo.BookData.name
          ? "File"
          : "Blob"
        : "";
    if (!BookDataFormat)
    ...
    if (BookDataFormat == "URI") {
      ...
      if (O.Local)
        return reject(
          `Bibi can't open books via ${
            D["book"] ? "data-bibi-book" : "URL"
          } on local mode`
        );
      B.Path = BookInfo.Book;
      const RootSource = (B.Type == "Zine" ? B.ZineData : B.Container).Source;
      const InitErrors = [],
        initialize_as = (FileOrFolder) => ({
          Promised: (FileOrFolder == "folder"
            ? O.download(RootSource).then(() => (B.PathDelimiter = "/") && "")
            : O.RangeLoader
            ? O.extract(RootSource).then(() => "on-the-fly")
            ...
            .catch((Err) => {
              InitErrors.push(
                (Err = /404/.test(String(Err))
                  ? Bibi.ErrorMessages.NotFound
                  : String(Err).replace(/^Error: /, ""))
              );
              O.log(
                `Failed as ${/^[aiueo]/i.test(B.Type) ? "an" : "a"} ${
                  B.Type
                } ${FileOrFolder}: ` + Err
              );
          ...
          or: function (fun) {
            return this.Promised.catch(fun);
          },
          or_reject: function (fun) {
            return this.or(() =>
              reject_failedToOpenTheBook(
                InitErrors.length < 2 || InitErrors[0] == InitErrors[1]
                  ? InitErrors[0]
      ...
      if (BookDataFormat == "File") {
        ...
        if (!S["accept-local-file"])
          return reject(`Local file is set to unacceptable`);
        if (!BookData.name)
          return reject(`File without a name is unacceptable`);
        if (!/\.[\w\d]+$/.test(BookData.name))
          return reject(`Local file without extension is set to unacceptable`);
        if (!O.isToBeExtractedIfNecessary(BookData.name))
          return reject(`File with this extension is set to unacceptable`);
        if (BookData.type) {
          if (
            /\.epub$/i.test(BookData.name)
              ? !MIMETypeREs["EPUB"].test(BookData.type)
              : /\.zip$/i.test(BookData.name)
              ? !MIMETypeREs["Zine"].test(BookData.type)
              : true
          )
            return reject(MIMETypeErrorMessage);
        }
        FileOrData = "file";
        ...
        if (BookDataFormat == "Base64") {
          ...
          if (!S["accept-base64-encoded-data"])
            return reject(`Base64 encoded data is set to unacceptable`);
          try {
            const Bin = atob(BookData.replace(/^.*,/, ""));
            const Buf = new Uint8Array(Bin.length);
            for (let l = Bin.length, i = 0; i < l; i++)
              Buf[i] = Bin.charCodeAt(i);
            BookData = new Blob([Buf.buffer], {
              type: BookInfo.BookDataMIMEType,
            });
          ...
          if (!S["accept-blob-converted-data"])
            return reject(`Blob converted data is set to unacceptable`);
    ...
    return (B.Type == "Zine"
      ? X.Zine.loadZineData()
      : L.loadContainer().then(L.loadPackage)
    )
      .then(() => E.dispatch("bibi:initialized-book"))
      .then(() => InitializedAs);
  });

L.loadContainer = () =>
  O.openDocument(B.Container.Source).then(L.loadContainer.process);

L.loadContainer.process = (Doc) =>
  (B.Package.Source.Path = Doc.getElementsByTagName("rootfile")[0].getAttribute(
    "full-path"
  ));

L.loadPackage = () =>
  O.openDocument(B.Package.Source).then(L.loadPackage.process);

L.loadPackage.process = (Doc) => {
  // This is Used also from the Zine Extention.
  const _Package = Doc.getElementsByTagName("package")[0];
  const _Metadata = Doc.getElementsByTagName("metadata")[0],
    Metadata = B.Package.Metadata;
  const _Manifest = Doc.getElementsByTagName("manifest")[0],
    Manifest = B.Package.Manifest;
  const _Spine = Doc.getElementsByTagName("spine")[0],
    Spine = B.Package.Spine;
  const SourcePaths = {};
  // ================================================================================
  // METADATA
  // --------------------------------------------------------------------------------
  const DCNS =
    _Package.getAttribute("xmlns:dc") || _Metadata.getAttribute("xmlns:dc");
  const UIDID = _Package.getAttribute("unique-identifier"),
    UIDE = UIDID ? Doc.getElementById(UIDID) : null,
    UIDTC = UIDE ? UIDE.textContent : "";
  Metadata["unique-identifier"] = UIDTC ? UIDTC.trim() : "";
  ["identifier", "language", "title", "creator", "publisher"].forEach((Pro) =>
    sML.forEach(Doc.getElementsByTagNameNS(DCNS, Pro))((_Meta) =>
      (Metadata[Pro] ? Metadata[Pro] : (Metadata[Pro] = [])).push(
        _Meta.textContent.trim()
      )
    )
  );
  ...
  if (!Metadata["identifier"])
    Metadata["identifier"] = Metadata["dcterms:identifier"] || [];
  if (!Metadata["language"])
    Metadata["language"] = Metadata["dcterms:language"] || ["en"];
  if (!Metadata["title"])
    Metadata["title"] = Metadata["dcterms:title"] || Metadata["identifier"];
  // --------------------------------------------------------------------------------
  if (!Metadata["rendition:layout"])
    Metadata["rendition:layout"] = "reflowable";
  if (Metadata["omf:version"]) Metadata["rendition:layout"] = "pre-paginated";
  if (
    !Metadata["rendition:orientation"] ||
    Metadata["rendition:orientation"] == "auto"
  )
    Metadata["rendition:orientation"] = "portrait";
  if (!Metadata["rendition:spread"] || Metadata["rendition:spread"] == "auto")
    Metadata["rendition:spread"] = "landscape";
  if (Metadata["original-resolution"])
    Metadata["original-resolution"] = O.getViewportByOriginalResolution(
      Metadata["original-resolution"]
    );
  if (Metadata["rendition:viewport"])
    Metadata["rendition:viewport"] = O.getViewportByMetaContent(
      Metadata["rendition:viewport"]
    );
  if (Metadata["fixed-layout-jp:viewport"])
    Metadata["fixed-layout-jp:viewport"] = O.getViewportByMetaContent(
      Metadata["fixed-layout-jp:viewport"]
    );
  if (Metadata["omf:viewport"])
    Metadata["omf:viewport"] = O.getViewportByMetaContent(
      Metadata["omf:viewport"]
    );
  B.ICBViewport =
    Metadata["original-resolution"] ||
    Metadata["rendition:viewport"] ||
  ...
  sML.forEach(_Manifest.getElementsByTagName("item"))((_Item) => {
    ...
    if (
      !Source["id"] ||
      !Source["href"] ||
      (!Source["media-type"] && B.Type == "EPUB")
    )
      return false;
    Source.Path = O.getPath(PackageDir, Source["href"]);
    if (Manifest[Source.Path])
      Source = Object.assign(Manifest[Source.Path], Source);
    ...
    if (Properties) {
      ...
      else if (Properties.includes("nav"))
        (B.Nav.Source = Source), (B.Nav.Type = "Navigation Document");
    }
  ...
  [B.Container, B.Package].forEach((Meta) => {
    if (Meta && Meta.Source) Meta.Source.Content = "";
  });
  // ================================================================================
  // SPINE
  // --------------------------------------------------------------------------------
  ...
  if (!B.PPD || !/^(ltr|rtl)$/.test(B.PPD))
    B.PPD = S["default-page-progression-direction"]; // default;
  // --------------------------------------------------------------------------------
  ...
  sML.forEach(_Spine.getElementsByTagName("itemref"))((ItemRef) => {
    ...
    const Source = Manifest[SourcePaths[IDRef]];
    if (!Source) return false;
    const Item = sML.create("iframe", {
      className: "item",
      
      scrolling: "no",
      allowtransparency:
        "true" /*, TimeCard: {}, stamp: function(What) { O.stamp(What, this.TimeCard); }*/,
    });
    Item["idref"] = IDRef;
    Item.Source = Source;
    Item.id ="data-block";
    ...
    if (Properties) {
      ...
      Properties.forEach((Pro) => {
        if (PropertyRE.test(Pro))
          ItemRef[Pro.replace(PropertyRE, "$1")] = Pro.replace(
            PropertyRE,
            "$4"
          );
      });
    }
    Item["rendition:layout"] =
      ItemRef["rendition:layout"] || Metadata["rendition:layout"];
    Item["rendition:orientation"] =
      ItemRef["rendition:orientation"] || Metadata["rendition:orientation"];
    Item["rendition:spread"] =
      ItemRef["rendition:spread"] || Metadata["rendition:spread"];
    Item["rendition:page-spread"] =
      ItemRef["rendition:page-spread"] || ItemRef["page-spread"] || undefined;
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    if (ItemRef.getAttribute("linear") == "no") {
      ...
      let Spread = null;
      if (
        Item["rendition:layout"] == "pre-paginated" &&
        Item["rendition:page-spread"] == SpreadAfter &&
        Item.Index > 0
      ) {
        const PreviousItem = R.Items[Item.Index - 1];
        if (
          Item["rendition:layout"] == "pre-paginated" &&
          PreviousItem["rendition:page-spread"] == SpreadBefore
        ) {
          PreviousItem.SpreadPair = Item;
          Item.SpreadPair = PreviousItem;
          Spread = Item.Spread = PreviousItem.Spread;
          Spread.Box.classList.remove(
            "single-item-spread-before",
            "single-item-spread-" + SpreadBefore
          );
          Spread.Box.classList.add(Item["rendition:layout"]);
        }
      }
      if (!Spread) {
        Spread = Item.Spread = sML.create("div", {
          className: "spread",
          Items: [],
          Pages: [],
          Index: R.Spreads.length,
        });
        Spread.Box = sML.create("div", {
          className: "spread-box " + Item["rendition:layout"],
          Inside: Spread,
          Spread: Spread,
        });
        if (Item["rendition:page-spread"]) {
          Spread.Box.classList.add(
            "single-item-spread-" + Item["rendition:page-spread"]
          );
          switch (Item["rendition:page-spread"]) {
            case SpreadBefore:
              Spread.Box.classList.add("single-item-spread-before");
              break;
            case SpreadAfter:
              Spread.Box.classList.add("single-item-spread-after");
              break;
          }
        }
        R.Spreads.push(
          SpreadsDocumentFragment.appendChild(Spread.Box).appendChild(Spread)
        );
      }
      Item.IndexInSpread = Spread.Items.push(Item) - 1;
      Item.Box = Spread.appendChild(
        sML.create("div", {
          className: "item-box " + Item["rendition:layout"],
          Inside: Item,
          Item: Item,
        })
      );
  ...
  B.FullTitle = FullTitleFragments.join(" - ")
    .replace(/&amp;?/gi, "&")
    .replace(/&lt;?/gi, "<")
    .replace(/&gt;?/gi, ">");
  O.Title.innerHTML = "";
  ...
  B.WritingMode = /^(zho?|chi|kor?|ja|jpn)$/.test(B.Language)
    ? B.PPD == "rtl"
      ? "tb-rl"
      : "lr-tb"
    ...
    ? "rl-tb"
    : /^(mo?n)$/.test(B.Language)
    ? "tb-lr"
    : "lr-tb";
  B.AllowPlaceholderItems =
    B.ExtractionPolicy != "at-once" &&
    Metadata["rendition:layout"] == "pre-paginated";
  // ================================================================================
  E.dispatch("bibi:processed-package");
};

L.createCover = () => {
  const VCover = I.Veil.Cover,
    PCover = I.Panel.BookInfo.Cover;
  VCover.Info.innerHTML = PCover.Info.innerHTML = (() => {
    const BookID = [];
    if (B.Title)
      BookID.push(`<strong>${L.createCover.optimizeString(B.Title)}</strong>`);
    if (B.Creator)
      BookID.push(`<em>${L.createCover.optimizeString(B.Creator)}</em>`);
    if (B.Publisher)
      BookID.push(`<span>${L.createCover.optimizeString(B.Publisher)}</span>`);
    return BookID.join(" ");
  })();
  return Promise.resolve(
    new Promise((resolve, reject) => {
      ...
      O.file(B.CoverImage.Source, { URI: true })
        .then((Item) => {
...
L.createCover.optimizeString = (Str) =>
  `<span>` + Str.replace(/([ 　・／]+)/g, "</span><span>$1") + `</span>`;

L.loadNavigation = () =>
  O.openDocument(B.Nav.Source).then((Doc) => {
    const PNav = (I.Panel.BookInfo.Navigation = I.Panel.BookInfo.insertBefore(
      sML.create("div", { id: "bibi-panel-bookinfo-navigation" }),
      I.Panel.BookInfo.firstElementChild
    ));
    PNav.innerHTML = "";
    ...
    if (B.Nav.Type == "Navigation Document") {
      sML.forEach(Doc.querySelectorAll("nav"))((Nav) => {
        switch (Nav.getAttribute("epub:type")) {
          case "toc":
            ...
            break;
          case "landmarks":
            Nav.classList.add("bibi-nav-landmarks");
            break;
          case "page-list":
            Nav.classList.add("bibi-nav-page-list");
            break;
        }
        sML.forEach(Nav.getElementsByTagName("*"))((Ele) =>
      ...
      const makeNavULTree = (Ele) => {
        ...
        for (let l = ChildNodes.length, i = 0; i < l; i++) {
          if (
            ...
            !/^navPoint$/i.test(ChildNodes[i].tagName)
          )
            continue;
          ...
          const LI = sML.create("li", { id: NavPoint.getAttribute("id") });
          LI.setAttribute("playorder", NavPoint.getAttribute("playorder"));
          const A = sML.create("a", {
            href: Content.getAttribute("src"),
            innerHTML: Text.innerHTML.trim(),
          });
      ...
      if (NavUL)
        NavContent.appendChild(document.createElement("nav")).appendChild(
          NavUL
        );
    }
...
L.coordinateLinkages = (BasePath, RootElement, InNav) => {
  const As = RootElement.getElementsByTagName("a");
  if (!As) return;
  for (let l = As.length, i = 0; i < l; i++) {
    ...
    if (/^[a-zA-Z]+:/.test(HRefPathInSource)) {
      if (HRefPathInSource.split("#")[0] == location.href.split("#")[0]) {
        ...
        HRefPathInSource = HRefHashInSource
          ? "#" + HRefHashInSource
          : R.Items[0].AnchorPath;
      } else {
        A.addEventListener("click", (Eve) => {
          Eve.preventDefault();
          Eve.stopPropagation();
    ...
    const HRefPath = O.getPath(
      BasePath.replace(/\/?([^\/]+)$/, ""),
      (!/^\.*\/+/.test(HRefPathInSource) ? "./" : "") +
        (/^#/.test(HRefPathInSource)
          ? BasePath.replace(/^.+?([^\/]+)$/, "$1")
          : "") +
        HRefPathInSource
    ...
    sML.forEach(R.Items)((Item) => {
      if (HRefFile == Item.AnchorPath) {
        ...
        if (Item["rendition:layout"] == "pre-paginated")
          A.Destination.PageIndexInItem = 0;
      ...
      new Promise((resolve) =>
        ...
        if (L.Opened) {
          ...
          return R.focusOn({
            ...
            I.History.add({ UI: B, SumUp: false, Destination: Destination })
          );
        }
        if (!L.Waiting) return false;
        if (S["start-in-new-window"])
          return L.openNewWindow(
            location.href +
              (location.hash ? "&" : "#") +
              "jo(nav=" +
              A.NavANumber +
...
L.preprocessResources = () => {
  ...
  const Promises = [],
    PreprocessedResources = [],
    pushItemPreprocessingPromise = (Item, URI) =>
      Promises.push(
        O.file(Item, { Preprocess: true, URI: URI }).then(() =>
          PreprocessedResources.push(Item)
        )
      );
  if (B.ExtractionPolicy)
    for (const FilePath in B.Package.Manifest) {
      const Item = B.Package.Manifest[FilePath];
      if (/\/(css|javascript)$/.test(Item["media-type"])) {
        // CSSs & JavaScripts in Manifest
  ...
  return Promise.all(Promises).then(() => {
    ...
    if (PreprocessedResources.length) {
      ...
      O.log(
        `Preprocessed. (${PreprocessedResources.length} Resource${
          PreprocessedResources.length > 1 ? "s" : ""
        })`,
        "</g>"
      );
    }
...
L.loadSpread = (Spread, Opt = {}) =>
  new Promise((resolve, reject) => {
    Spread.AllowPlaceholderItems =
      S["allow-placeholders"] && Opt.AllowPlaceholderItems;
    let LoadedItemsInSpread = 0,
      SkippedItemsInSpread = 0;
    Spread.Items.forEach((Item) => {
      L.loadItem(Item, { IsPlaceholder: Opt.AllowPlaceholderItems })
        ...
        .then(() => {
          if (LoadedItemsInSpread + SkippedItemsInSpread == Spread.Items.length)
            /*(SkippedItemsInSpread ? reject : resolve)*/ resolve(Spread);
        });
    });
  });
...
L.loadItem = (Item, Opt = {}) => {
  ...
  if (IsPlaceholder === Item.IsPlaceholder)
    return Item.Loading ? Item.Loading : Promise.resolve(Item);
  ...
  return (Item.Loading = (Item.IsPlaceholder // Promise
    ? Promise.reject()
    : Item.ContentURL
    ? Promise.resolve()
    ...
    ? O.file(Item.Source, {
        ...
        initialize: () => {
          if (!S["allow-scripts-in-content"]) {
            Item.Source.Content = Item.Source.Content.replace(
              /<script(\s+[\w\-]+(\s*=\s*('[^'']*'|"[^""]*"))?)*\s*\/>/gi,
              ""
            );
      ...
      }).then((ItemSource) => ItemSource.Content)
    : /\.(gif|jpe?g|png)$/i.test(Item.Source.Path) // Bitmap-in-Spine
    ? O.file(Item.Source, {
        URI: true,
      }).then((ItemSource) => [
        Item["rendition:layout"] == "pre-paginated" && B.ICBViewport
          ? `<meta name="viewport" content="width=${B.ICBViewport.Width}, height=${B.ICBViewport.Height}" />`
          : "",
    ...
    ? O.file(Item.Source, {
        ...
        initialize: () => {
          const StyleSheetRE = /<\?xml-stylesheet\s*(.+?)\s*\?>/g,
            MatchedStyleSheets = Item.Source.Content.match(StyleSheetRE);
          if (!S["allow-scripts-in-content"])
            O.sanitizeItemSource(Item.Source, { As: "SVG" });
          Item.Source.Content =
            (MatchedStyleSheets
              ? MatchedStyleSheets.map((SS) =>
                  SS.replace(StyleSheetRE, `<link rel="stylesheet" $1 />`)
                ).join("")
              : "") +
            "<bibi:boundary/>" +
            Item.Source.Content; // Join for preprocessing.
        },
      }).then((ItemSource) => ItemSource.Content.split("<bibi:boundary/>"))
    ...
    .then(
      (ItemSourceContent) =>
        new Promise((resolve) => {
          ...
          if (!Item.ContentURL) {
            let HTML =
              typeof ItemSourceContent == "string"
                ? ItemSourceContent
                : [
                    `<!DOCTYPE html>`,
                    `<html>`,
                    `<head>`,
                    `<meta charset="utf-8" />`,
                    `<title>${B.FullTitle} - #${Item.Index + 1}/${
                      R.Items.length
                    }</title>`,
            ...
            HTML = HTML.replace(
              /(<head(\s[^>]+)?>)/i,
              `$1\n<link rel="stylesheet" id="${DefaultStyleID}" href="${Bibi.BookStyleURL}" />` +
                (!B.ExtractionPolicy && !Item.Source.Preprocessed
                  ? `\n<base href="${O.fullPath(Item.Source.Path)}" />`
                  : "")
            );
            if (O.Local || sML.UA.LINE || sML.UA.Trident || sML.UA.EdgeHTML) {
              // Legacy Microsoft Browsers do not accept DataURLs for src of <iframe>. Also LINE in-app-browser is probably the same as it.
              HTML = HTML.replace(/^<\?.+?\?>/, "").replace(
                "</head>",
                `<script id="bibi-onload">window.addEventListener('load', function() { parent.R.Items[${Item.Index}].onLoaded(); return false; });</script>\n</head>`
              );
              Item.onLoaded = () => {
                resolve();
                Item.contentDocument.head.removeChild(
                  Item.contentDocument.getElementById("bibi-onload")
                );
            ...
            (Item.ContentURL = O.createBlobURL(
              "Text",
              HTML,
              S["allow-scripts-in-content"] &&
                /\.(xht(ml)?|xml)$/i.test(Item.Source.Path)
...
L.postprocessItem = (Item) => {
  ...
  Item.HTML.classList.add(...sML.Environments);
  ...
  const XMLLang = Item.HTML.getAttribute("xml:lang"),
    Lang = Item.HTML.getAttribute("lang");
  if (!XMLLang && !Lang)
    Item.HTML.setAttribute("xml:lang", B.Language),
      Item.HTML.setAttribute("lang", B.Language);
  else if (!XMLLang) Item.HTML.setAttribute("xml:lang", Lang);
  else if (!Lang) Item.HTML.setAttribute("lang", XMLLang);
  sML.forEach(Item.Body.getElementsByTagName("link"))((Link) =>
    Item.Head.appendChild(Link)
  );
  sML.appendCSSRule(
  ...
  if (sML.UA.Trident)
    sML.forEach(Item.Body.getElementsByTagName("svg"))((SVG) => {
      ...
      if (ChildImages.length == 1) {
        ...
        if (
          ChildImage.getAttribute("width") &&
          ChildImage.getAttribute("height")
        ) {
  ...
  const Lv1Eles = Item.contentDocument.querySelectorAll(
    "body>*:not(script):not(style)"
  );
  if (Lv1Eles && Lv1Eles.length == 1) {
    const Lv1Ele = Item.contentDocument.querySelector(
      "body>*:not(script):not(style)"
    );
    if (/^svg$/i.test(Lv1Ele.tagName))
      Item.Outsourcing = Item.OnlySingleSVG = true;
    else if (/^img$/i.test(Lv1Ele.tagName))
      Item.Outsourcing = Item.OnlySingleIMG = true;
    else if (/^iframe$/i.test(Lv1Ele.tagName)) Item.Outsourcing = true;
    else if (!O.getElementInnerText(Item.Body)) Item.Outsourcing = true;
  }
  return (Item["rendition:layout"] == "pre-paginated"
    ? Promise.resolve()
    : L.patchItemStyles(Item)
  ).then(() => {
...
L.patchItemStyles = (Item) =>
  new Promise((resolve) => {
    ...
    sML.forEach(Item.HTML.querySelectorAll("link, style"))((SSEle) => {
      if (/^link$/i.test(SSEle.tagName)) {
        ...
        if (
          ...
          SSEle.rel == "alternate stylesheet"
        )
          return; //// Safari does not count "alternate stylesheet" in document.styleSheets.
      }
      Item.StyleSheets.push(SSEle);
    });
    const checkCSSLoadingAndResolve = () => {
      if (Item.contentDocument.styleSheets.length < Item.StyleSheets.length)
        return false;
    ...
    if (!checkCSSLoadingAndResolve())
      Item.CSSLoadingTimerID = setInterval(checkCSSLoadingAndResolve, 33);
  }).then(() => {
    if (!Item.Source.Preprocessed) {
      if (B.Package.Metadata["ebpaj:guide-version"]) {
        ...
        if (
          ...
          Item.Body.style.textUnderlinePosition = "under left";
      }
      if (sML.UA.Trident) {
        ...
        O.editCSSRules(Item.contentDocument, (CSSRule) => {
          if (/(-(epub|webkit)-)?column-count: 1; /.test(CSSRule.cssText))
            CSSRule.style.columnCount = CSSRule.style.msColumnCount = "auto";
          if (
          ...
          if (
            ...
            CSSRule.style.writingMode = "tb-lr";
          if (
            /(-(epub|webkit)-)?writing-mode: horizontal-tb; /.test(
              CSSRule.cssText
            )
          )
            CSSRule.style.writingMode = "lr-tb";
          if (
            /(-(epub|webkit)-)?(text-combine-upright|text-combine-horizontal): all; /.test(
              CSSRule.cssText
            )
          )
            CSSRule.style.msTextCombineHorizontal = "all";
          if (IsCJK && / text-align: justify; /.test(CSSRule.cssText))
            CSSRule.style.textJustify = "inter-ideograph";
        });
      } else {
        O.editCSSRules(Item.contentDocument, (CSSRule) => {
          if (/(-(epub|webkit)-)?column-count: 1; /.test(CSSRule.cssText))
            CSSRule.style.columnCount = CSSRule.style.webkitColumnCount =
              "auto";
        });
      }
    }
    ...
    if (
      ItemHTMLComputedStyle[O.WritingModeProperty] !=
      ItemBodyComputedStyle[O.WritingModeProperty]
    )
      Item.HTML.style.writingMode =
        ItemBodyComputedStyle[O.WritingModeProperty];
    Item.WritingMode = O.getWritingMode(Item.HTML);
    if (/-rl$/.test(Item.WritingMode))
      Item.HTML.classList.add("bibi-vertical-text");
    else if (/-lr$/.test(Item.WritingMode))
      Item.HTML.classList.add("bibi-horizontal-text");
    /*
...
R.createSpine = (SpreadsDocumentFragment) => {
  R.Main = O.Body.insertBefore(
    sML.create("main", { id: "bibi-main" }),
    O.Body.firstElementChild
  );
...
R.resetStage = () => {
  ...
  R.Main.Book.style.padding = R.Main.Book.style.width = R.Main.Book.style.height =
    "";
  const BookBreadthIsolationStartEnd =
    (S["use-slider"] && S.RVM == "paged" && O.Scrollbars[C.A_SIZE_B]
      ? O.Scrollbars[C.A_SIZE_B]
      : 0) +
    S["spread-margin"] * 2;
  sML.style(R.Main.Book, {
    [C.A_SIZE_b]:
      BookBreadthIsolationStartEnd > 0
        ? "calc(100% - " + BookBreadthIsolationStartEnd + "px)"
        : "",
    [C.A_SIZE_l]: "",
  });
  ...
  R.Stage.Height = WIH;
  R.Stage[C.A_SIZE_B] -=
    (S["use-slider"] || S.RVM != "paged" ? O.Scrollbars[C.A_SIZE_B] : 0) +
    S["spread-margin"] * 2;
  window.scrollTo(0, 0);
  if (!S["use-full-height"]) R.Stage.Height -= I.Menu.Height;
  if (S["spread-margin"] > 0)
    R.Main.Book.style["padding" + C.L_BASE_S] = R.Main.Book.style[
      "padding" + C.L_BASE_E
    ] = S["spread-margin"] + "px";
...
R.layOutSpreadAndItsItems = (Spread, Opt = {}) => {
  ...
  return (Spread.Items.length == 1
    ? R.layOutItem(Spread.Items[0])
    : !Opt.Reverse
    ? R.layOutItem(Spread.Items[0]).then(() => R.layOutItem(Spread.Items[1]))
    : R.layOutItem(Spread.Items[1]).then(() => R.layOutItem(Spread.Items[0]))
  ).then(() => R.layOutSpread(Spread, Opt));
};

R.layOutSpread = (Spread, Opt = {}) =>
  new Promise((resolve) => {
    if (Opt.Makeover) {
    ...
    Spread.Items.forEach((Item) =>
      Item.Pages.forEach(
        (Page) => (Page.IndexInSpread = Spread.Pages.push(Page) - 1)
      )
    );
    const SpreadSize = { Width: 0, Height: 0 },
      SpreadBox = Spread.Box;
    if (Spread.Items.length == 1) {
      ...
      SpreadSize.Width =
        ...
        Item["rendition:page-spread"]
          ? B.ICBViewport
            ? (Item.Box.offsetHeight * B.ICBViewport.Width * 2) /
              B.ICBViewport.Height
            : R.Stage.Width
          : Item.Box.offsetWidth;
      SpreadSize.Height = Item.Box.offsetHeight;
    } else {
      const ItemA = Spread.Items[0],
        ItemB = Spread.Items[1];
      Spread.Spreaded = ItemA.Spreaded || ItemB.Spreaded ? true : false;
      if (
        ItemA["rendition:layout"] == "pre-paginated" &&
        ItemB["rendition:layout"] == "pre-paginated"
      ) {
        // Paired Pre-Paginated Items
        if (Spread.Spreaded || S.RVM != "vertical") {
          // Spreaded
          SpreadSize.Width = ItemA.Box.offsetWidth + ItemB.Box.offsetWidth;
          SpreadSize.Height = Math.max(
            ItemA.Box.offsetHeight,
            ItemB.Box.offsetHeight
          );
        } else {
          // Not Spreaded (Vertical)
          SpreadSize.Width = Math.max(
            ItemA.Box.offsetWidth,
            ItemB.Box.offsetWidth
          );
        ...
        if (S.SLA == "horizontal") {
          // if(R.Stage.Width > ItemA.Box.offsetWidth + ItemB.Box.offsetWidth) {
          // horizontal layout
          SpreadSize.Width = ItemA.Box.offsetWidth + ItemB.Box.offsetWidth;
          SpreadSize.Height = Math.max(
            ItemA.Box.offsetHeight,
          ...
          if (Bibi.Dev) {
            ...
            O.log(
              `[0] w${ItemA.Box.offsetWidth}/h${ItemA.Box.offsetHeight} %O`,
              ItemA
            );
            O.log(
              `[1] w${ItemB.Box.offsetWidth}/h${ItemB.Box.offsetHeight} %O`,
              ItemB
            );
            O.log(
              `-=> w${SpreadSize.Width}/h${SpreadSize.Height} %O`,
              Spread,
              "</g>"
            );
          }
    ...
    const MinSpaceOfEdge =
      S.RVM != "paged" &&
      (Spread.Index == 0 || Spread.Index == R.Spreads.length - 1)
        ? Math.floor((R.Stage[C.L_SIZE_L] - SpreadSize[C.L_SIZE_L]) / 2)
        : 0;
    if (MinSpaceOfEdge > 0) {
      if (Spread.Index == 0)
        (Spread.style["padding" + C.L_BASE_B] = MinSpaceOfEdge + "px"),
          (SpreadSize[C.L_SIZE_L] += MinSpaceOfEdge);
      if (Spread.Index == R.Spreads.length - 1)
        (Spread.style["padding" + C.L_BASE_A] = MinSpaceOfEdge + "px"),
          (SpreadSize[C.L_SIZE_L] += MinSpaceOfEdge);
    } else {
      Spread.style.padding = "";
    }
    if (O.Scrollbars.Height && S.SLA == "vertical" && S.ARA != "vertical") {
      SpreadBox.style.minHeight =
        S.RVM == "paged" ? "calc(100vh - " + O.Scrollbars.Height + "px)" : "";
      SpreadBox.style.marginBottom =
        Spread.Index == R.Spreads.length - 1 ? O.Scrollbars.Height + "px" : "";
    } else {
      SpreadBox.style.minHeight = SpreadBox.style.marginBottom = "";
    }
    SpreadBox.classList.toggle("spreaded", Spread.Spreaded);
    (SpreadBox.style[C.L_SIZE_b] = ""),
      (Spread.style[C.L_SIZE_b] = Math.ceil(SpreadSize[C.L_SIZE_B]) + "px");
    SpreadBox.style[C.L_SIZE_l] = Spread.style[C.L_SIZE_l] =
      Math.ceil(SpreadSize[C.L_SIZE_L]) + "px";
    ...
    if (Opt.Makeover) {
      ...
      const ChangedSpreadBoxLength =
        Spread.Box["offset" + C.L_SIZE_L] - Spread.PreviousSpreadBoxLength;
      if (ChangedSpreadBoxLength != 0) {
        const Correct =
          C.L_AXIS_L == "X" &&
          O.getElementCoord(Spread, R.Main).X + Spread.offsetWidth <=
            R.Main.scrollLeft + R.Main.offsetWidth;
        R.Main.Book.style[C.L_SIZE_l] =
          parseFloat(getComputedStyle(R.Main.Book)[C.L_SIZE_l]) +
          ChangedSpreadBoxLength +
          "px";
        if (Correct) {
...
R.renderReflowableItem = (Item) => {
  const ItemPaddingSE =
    S["item-padding-" + C.L_BASE_s] + S["item-padding-" + C.L_BASE_e];
  const ItemPaddingBA =
    S["item-padding-" + C.L_BASE_b] + S["item-padding-" + C.L_BASE_a];
  ...
  ["b", "a", "s", "e"].forEach((base) => {
    const trbl = C["L_BASE_" + base];
    Item.style["padding-" + trbl] = S["item-padding-" + trbl] + "px";
  });
  ...
  const ReverseItemPaginationDirectionIfNecessary =
    sML.UA.Trident || sML.UA.EdgeHTML ? false : true;
  if (Item.Columned) {
    sML.style(Item.HTML, {
      "column-fill": "",
      "column-width": "",
    ...
    if (ReverseItemPaginationDirectionIfNecessary && Item.ReversedColumned) {
      ...
      sML.forEach(Item.contentDocument.querySelectorAll("body > *"))(
        (Ele) => (Ele.style.direction = "")
      );
    }
  }
  ...
  Item.Spreaded =
    S.SLA == "horizontal" &&
    (S["pagination-method"] == "x" || /-tb$/.test(Item.WritingMode)) &&
    (Item["rendition:spread"] == "both" ||
      R.Orientation == Item["rendition:spread"] ||
  ...
  const WordWrappingStyleSheetIndex = sML.appendCSSRule(
    Item.contentDocument,
    "*",
  ...
  sML.forEach(Item.Body.querySelectorAll("img, svg"))((Img) => {
    ...
    ["width", "height", "maxWidth", "maxHeight"].forEach(
      (Pro) => (Img.style[Pro] = Img.BibiDefaultStyle[Pro])
    );
    if (
      (S.RVM == "horizontal" && /-(rl|lr)$/.test(Item.WritingMode)) ||
      (S.RVM == "vertical" && /-tb$/.test(Item.WritingMode))
    )
      return;
    const NaturalB = parseFloat(getComputedStyle(Img)[C.L_SIZE_b]),
      MaxB = Math.floor(
        Math.min(parseFloat(getComputedStyle(Item.Body)[C.L_SIZE_b]), PageCB)
      );
    const NaturalL = parseFloat(getComputedStyle(Img)[C.L_SIZE_l]),
      MaxL = Math.floor(
        Math.min(parseFloat(getComputedStyle(Item.Body)[C.L_SIZE_l]), PageCL)
      );
    if (NaturalB > MaxB || NaturalL > MaxL)
      sML.style(Img, {
        [C.L_SIZE_b]:
          Math.floor(
            parseFloat(getComputedStyle(Img)[C.L_SIZE_b]) *
              Math.min(MaxB / NaturalB, MaxL / NaturalL)
          ) + "px",
        [C.L_SIZE_l]: "auto",
        maxWidth: "100vw",
        maxHeight: "100vh",
  ...
  if (!Item.Outsourcing) {
    if (S["pagination-method"] == "x") {
      if (S.RVM == "paged" && Item.HTML["offset" + C.L_SIZE_L] > PageCL)
        PaginateWith = "S";
      // VM:Paged            WM:Vertical   LA:Horizontal
    ...
    } else if (S.RVM == "paged" || Item.HTML["offset" + C.L_SIZE_B] > PageCB)
      PaginateWith = "C"; // VM:Paged/Horizontal WM:Horizontal LA:Horizontal // VM:Paged/Vertical WM:Vertical LA:Vertical
  }
  switch (PaginateWith) {
    case "S":
      ...
      const HowManyPages = Math.ceil(
        (sML.UA.Trident ? Item.Body.clientHeight : Item.HTML.scrollHeight) /
          PageCB
      );
      sML.style(Item.HTML, {
        width: "",
      ...
      for (let i = 1; i < HowManyPages; i++) {
        const Start = 0,
          End = PageCB,
          After = (PageCL + PageGap) * i,
          Before = After - PageGap;
      ...
      const Neck = Bibi.createElement("bibi-neck"),
        Throat = Neck.appendChild(Bibi.createElement("bibi-throat")),
        ShadowOrThroat = Throat.attachShadow
          ? Throat.attachShadow({ mode: "open" })
          : Throat.createShadowRoot
          ? Throat.createShadowRoot()
          : Throat;
      ShadowOrThroat.appendChild(document.createElement("style")).textContent =
        (ShadowOrThroat != Throat ? ":host" : "bibi-throat") +
        ` { ${C.L_SIZE_b}: ${PageCB}px; ${
          C.L_SIZE_l
        }: ${ItemLength}px; shape-outside: polygon(${Polygon.join(", ")}); }`;
    ...
    case "C":
      ...
      R.Columned = true;
      (Item.Columned = true),
        (Item.ColumnBreadth = PageCB),
        (Item.ColumnLength = PageCL);
      if (ReverseItemPaginationDirectionIfNecessary) {
        let ToBeReversedColumnAxis = false;
        switch (Item.WritingMode) {
          case "lr-tb":
          case "tb-lr":
            if (S["page-progression-direction"] != "ltr")
              ToBeReversedColumnAxis = true;
            break;
          case "rl-tb":
          case "tb-rl":
        ...
        if (ToBeReversedColumnAxis) {
          ...
          sML.forEach(Item.contentDocument.querySelectorAll("body > *"))(
            (Ele) => (Ele.style.direction = getComputedStyle(Ele).direction)
          );
  ...
  let ItemL = sML.UA.Trident
    ? Item.Body["client" + C.L_SIZE_L]
    : Item.HTML["scroll" + C.L_SIZE_L];
  ...
  for (let i = 0; i < HowManyPages; i++) {
    const Page = Item.Box.appendChild(
      sML.create("span", { className: "page" })
    );
...
R.renderPrePaginatedItem = (Item) => {
  ...
  if (Item.Spreaded) {
    ...
    if (Item.SpreadPair) {
      ...
      let LoBaseItem = null,
        LoBaseItemLoVp = null; // LayoutBaseItem, LayoutBaseItemLayoutViewport
      let LoPairItem = null,
        LoPairItemLoVp = null; // LayoutPairItem, LayoutPairItemLayoutViewport
      if (PairItem.Index > Item.Index)
        (LoBaseItem = Item),
          (LoBaseItemLoVp = ItemLoVp),
          (LoPairItem = PairItem),
          (LoPairItemLoVp = PairItemLoVp);
      else
        (LoBaseItem = PairItem),
          (LoBaseItemLoVp = PairItemLoVp),
          (LoPairItem = Item),
          (LoPairItemLoVp = ItemLoVp);
      ...
      const SpreadViewPort = {
        Width:
          ItemLoVp.Width *
          (/^(left|right)$/.test(Item["rendition:page-spread"]) ? 2 : 1),
...
R.getItemViewport = (Item) =>
  Item.IsPlaceholder
    ...
    : (() => {
        ...
        if (ViewportMeta)
          return O.getViewportByMetaContent(
            ViewportMeta.getAttribute("content")
          );
        if (Item.OnlySingleSVG)
          return O.getViewportByViewBox(
            Item.Body.firstElementChild.getAttribute("viewBox")
          ); // It's also for Item of SVGs-in-Spine, or Fixed-Layout Item including SVG without Viewport.
        if (Item.OnlySingleIMG)
          return O.getViewportByImage(Item.Body.firstElementChild); // It's also for Item of Bitmaps-in-Spine.
        return null;
      })();
...
R.getItemLayoutViewport = (Item) =>
  Item.Viewport
    ...
    : {
        Width:
          (R.Stage.Height * S["orientation-border-ratio"]) /
          /*Item.Spreaded &&*/ (/^(left|right)$/.test(
            Item["rendition:page-spread"]
          )
            ? 2
            : 1),
...
R.organizePages = () =>
  (R.Pages = R.Spreads.reduce(
    (NewPages, Spread) =>
      Spread.Pages.reduce((NewPages, Page) => {
        Page.Index = NewPages.push(Page) - 1;
        return NewPages;
      }, NewPages),
    []
  ));
...
R.replacePages = (OldPages, NewPages) => {
  const StartIndex = OldPages[0].Index,
    OldLength = OldPages.length,
    NewLength = NewPages.length;
  for (let l = NewPages.length, i = 0; i < l; i++)
    NewPages[i].Index = StartIndex + i;
...
R.layOutStage = () => {
  ...
  R.Spreads.forEach(
    (Spread) => (MainContentLayoutLength += Spread.Box["offset" + C.L_SIZE_L])
  );
  if (
    S["book-rendition-layout"] == "pre-paginated" ||
    S["reader-view-mode"] != "paged"
  )
...
R.layOutBook = (Opt) =>
  new Promise((resolve, reject) => {
    ...
    if (!Opt.Reset) {
      ...
      const Promises = [];
      R.Spreads.forEach((Spread) =>
        Promises.push(R.layOutSpreadAndItsItems(Spread))
      );
...
R.updateOrientation = () => {
  ...
  if (typeof window.orientation != "undefined") {
    ...
    const W =
      window.innerWidth - (S.ARA == "vertical" ? O.Scrollbars.Width : 0);
    const H =
      window.innerHeight - (S.ARA == "horizontal" ? O.Scrollbars.Height : 0);
    R.Orientation =
      W / H < S["orientation-border-ratio"] ? "portrait" : "landscape";
  }
...
R.changeView = (Par) => {
  if (
    ...
    !Par ||
    typeof Par.Mode != "string" ||
    !/^(paged|horizontal|vertical)$/.test(Par.Mode) ||
  ...
  if (L.Opened) {
    ...
    const Delay = [O.TouchOS ? 99 : 3, O.TouchOS ? 99 : 33];
    setTimeout(() => {
  ...
  if (S["keep-settings"] && O.Biscuits)
    O.Biscuits.memorize("Book", { RVM: Par.Mode });
};
...
R.focusOn = (Par) =>
  new Promise((resolve, reject) => {
    ...
    if (typeof Par == "number" || /^\d+$/.test(Par))
      Par = { Destination: Par * 1 };
    ...
    if (S["book-rendition-layout"] == "reflowable") {
      if (typeof _.Point == "number") {
        ...
        if (_.Side == "after")
          Par.FocusPoint +=
            (_.Page["offset" + C.L_SIZE_L] - R.Stage[C.L_SIZE_L]) * C.L_AXIS_D;
      ...
      if (R.Stage[C.L_SIZE_L] >= _.Page.Spread["offset" + C.L_SIZE_L]) {
        ...
        Par.FocusPoint -= Math.floor(
          (R.Stage[C.L_SIZE_L] - _.Page.Spread["offset" + C.L_SIZE_L]) / 2
        );
      } else {
        Par.FocusPoint = O.getElementCoord(_.Page)[C.L_AXIS_L];
        if (R.Stage[C.L_SIZE_L] > _.Page["offset" + C.L_SIZE_L])
          Par.FocusPoint -= Math.floor(
            (R.Stage[C.L_SIZE_L] - _.Page["offset" + C.L_SIZE_L]) / 2
          );
        else if (_.Side == "after")
          Par.FocusPoint +=
            (_.Page["offset" + C.L_SIZE_L] - R.Stage[C.L_SIZE_L]) * C.L_AXIS_D;
      }
    }
    ...
    ScrollTarget[C.L_AXIS_L] = Par.FocusPoint;
    if (!S["use-full-height"] && S.RVM == "vertical")
      ScrollTarget.Y -= I.Menu.Height;
    return sML
      .scrollTo(ScrollTarget, {
        ForceScroll: true,
        Duration:
          typeof Par.Duration == "number"
            ? Par.Duration
            : S.SLA == S.ARA && S.RVM != "paged"
            ? 222
            : 0,
...
R.hatchDestination = (_) => {
  ...
  if (
    S.BRL == "reflowable" &&
    ((S.SLA == "vertical" && /-tb$/.test(B.WritingMode)) ||
      (S.SLA == "horizontal" && /^tb-/.test(B.WritingMode))) &&
  ...
  delete _.Point;
  if (_.Page) {
    if (R.Pages[_.Page.Index] != _.Page) delete _.Page;
    // Pages of the Item have been replaced.
    else return _;
  }
...
R.hatchPage = (_) => {
  ...
  if (
    typeof _.SIPP == "number" &&
    ((typeof _.PageIndexInSpread != "number" &&
      typeof _.PageProgressInSpread != "number") ||
      (typeof _.SpreadIndex != "number" && !_.Spread))
  )
    (_.SpreadIndex = Math.floor(_.SIPP)),
      (_.PageProgressInSpread = String(_.SIPP).replace(/^\d*\./, "0.") * 1);
  if (
    typeof _.IIPP == "number" &&
    ((typeof _.PageIndexInItem != "number" &&
      typeof _.PageProgressInItem != "number") ||
      (typeof _.ItemIndex != "number" && !_.Item))
  )
    (_.ItemIndex = Math.floor(_.IIPP)),
      (_.PageProgressInItem = String(_.IIPP).replace(/^\d*\./, "0.") * 1);
  ...
  try {
    if (typeof _.ElementSelector == "string") {
      ...
      if (_.Item)
        _.Element = _.Item.contentDocument.querySelector(_.ElementSelector);
    ...
    if (typeof _.PageIndexInItem == "number")
      return R.hatchItem(_).Pages[_.PageIndexInItem];
    if (typeof _.PageIndexInSpread == "number")
      return R.hatchSpread(_).Pages[_.PageIndexInSpread];
    if (typeof _.PageProgressInItem == "number")
      return ((Item) =>
        Item.Pages[
          sML.limitMax(
            Math.floor(Item.Pages.length * _.PageProgressInItem),
            Item.Pages.length - 1
          )
        ])(R.hatchItem(_));
    if (typeof _.PageProgressInSpread == "number")
      return ((Spread) =>
        Spread.Pages[
          sML.limitMax(
            Math.floor(Spread.Pages.length * _.PageProgressInSpread),
...
R.hatchItem = (_) => {
  ...
  if (typeof _.ItemIndexInSpine == "number")
    return B.Package.Spine[_.ItemIndexInSpine];
  if (typeof _.ItemIndexInSpread == "number")
    try {
      return R.hatchSpread(_).Items[_.ItemIndexInSpread];
    } catch (_) {
      return null;
    }
...
R.hatchNearestPageOfElement = (Ele) => {
  ...
  if (Item.Columned) {
    ...
    if (S.PPD == "rtl" && S.SLA == "vertical")
      ElementCoordInItem =
        /* !!!! Use offsetWidth of **Body** >>>> */ Item.Body.offsetWidth -
        ElementCoordInItem -
        Ele.offsetWidth;
    NearestPage =
      Item.Pages[
        ElementCoordInItem <= 0
          ? 0
          : Math.floor(ElementCoordInItem / Item.ColumnBreadth)
      ];
  } else {
    ElementCoordInItem = O.getElementCoord(Ele)[C.L_AXIS_L];
    if (S.PPD == "rtl" && S.SLA == "horizontal")
      ElementCoordInItem =
        /* !!!! Use offsetWidth of **HTML** >>>> */ Item.HTML.offsetWidth -
        ElementCoordInItem -
        Ele.offsetWidth;
    NearestPage =
      Item.Pages[Math.floor(ElementCoordInItem / R.Stage[C.L_SIZE_L])];
    /*
...
R.hatchPoint = (_) => {
  try {
    if (typeof _.P == "string" && _.P) Object.assign(_, R.getPDestination(_.P));
    if (typeof _.ElementSelector == "string") {
      _.Element = R.hatchItem(_).contentDocument.querySelector(
        _.ElementSelector
      );
      delete _.ElementSelector;
    }
...
R.hatchPointOfElement = (Ele) => {
  ...
  if (Item.Columned) {
    ...
    if (S.PPD == "rtl" && S.SLA == "vertical")
      ElementCoordInItem =
        Item.Body.offsetWidth - ElementCoordInItem - Ele.offsetWidth; // Use offsetWidth of **Body**
  } else {
    ElementCoordInItem = O.getElementCoord(Ele)[C.L_AXIS_L];
    if (S.PPD == "rtl" && S.SLA == "horizontal")
      ElementCoordInItem += Ele.offsetWidth; // Use offsetWidth of **Body**
  }
  return (
    O.getElementCoord(Item)[C.L_AXIS_L] +
    S["item-padding-" + C.L_OOBL_l] +
    ElementCoordInItem
  );
};
...
R.getPDestination = (PString) => {
  ...
  if (/^[a-z]+$/.test(PString))
    return PString == "head" || PString == "foot" ? { Edge: PString } : null;
  ...
  if (Steps.length) {
    if (/^[a-z]+$/.test(Steps[0])) {
      ...
      Steps.forEach(
        (Step) => (ElementSelector += `>*:nth-child(` + Step + `)`)
      );
...
R.selectTextLocation = (_) => {
  ...
  const Sides = {
    Start: { Node: _Node, Index: 0 },
    End: { Node: _Node, Index: _Node.textContent.length },
  };
  if (_.TermStep) {
    if (_.TermStep.Preceding || _.TermStep.Following) {
      (Sides.Start.Index = _.TermStep.Index),
        (Sides.End.Index = _.TermStep.Index);
      if (_.TermStep.Preceding)
        Sides.Start.Index -= _.TermStep.Preceding.length;
      if (_.TermStep.Following) Sides.End.Index += _.TermStep.Following.length;
      if (Sides.Start.Index < 0 || _Node.textContent.length < Sides.End.Index)
        return;
      if (
        _Node.textContent.substr(
          Sides.Start.Index,
          Sides.End.Index - Sides.Start.Index
        ) !=
        _.TermStep.Preceding + _.TermStep.Following
      )
        return;
    } else if (_.TermStep.Side && _.TermStep.Side == "a") {
      Sides.Start.Node = _Node.parentNode.firstChild;
      while (Sides.Start.Node.childNodes.length)
        Sides.Start.Node = Sides.Start.Node.firstChild;
      Sides.End.Index = _.TermStep.Index - 1;
    } else {
      Sides.Start.Index = _.TermStep.Index;
      Sides.End.Node = _Node.parentNode.lastChild;
      while (Sides.End.Node.childNodes.length)
        Sides.End.Node = Sides.End.Node.lastChild;
...
R.moveBy = (Par) =>
  new Promise((resolve, reject) => {
    ...
    if (
      typeof Par.Distance != "number" ||
      !isFinite(Par.Distance) ||
      !Par.Distance
    )
      return reject();
    //Par.Distance = Par.Distance < 0 ? -1 : 1;
    E.dispatch("bibi:is-going-to:move-by", Par);
    const Current = (Par.Distance > 0
        ? I.PageObserver.Current.List.slice(-1)
        : I.PageObserver.Current.List)[0],
      CurrentPage = Current.Page,
      CurrentItem = CurrentPage.Item;
    let Promised = null;
    if (
      true ||
      R.Columned ||
      ...
      (Par.Distance > 0 &&
        CurrentPage.IndexInItem == CurrentItem.Pages.length - 1)
    ) {
      let Side = Par.Distance > 0 ? "before" : "after";
      if (Current.PageIntersectionStatus.Oversized) {
        if (Par.Distance > 0) {
          if (Current.PageIntersectionStatus.Entering)
            (Par.Distance = 0), (Side = "before");
          else if (Current.PageIntersectionStatus.Headed)
            (Par.Distance = 0), (Side = "after");
        } else {
          if (Current.PageIntersectionStatus.Footed)
            (Par.Distance = 0), (Side = "before");
          else if (Current.PageIntersectionStatus.Passing)
            (Par.Distance = 0), (Side = "before");
        }
      } else {
        if (Par.Distance > 0) {
          ...
          if (Current.PageIntersectionStatus.Passing)
            (Par.Distance = 0), (Side = "before");
        }
      }
      let DestinationPageIndex = CurrentPage.Index + Par.Distance;
      if (DestinationPageIndex < 0)
        (DestinationPageIndex = 0), (Side = "before");
      else if (DestinationPageIndex > R.Pages.length - 1)
        (DestinationPageIndex = R.Pages.length - 1), (Side = "after");
      let DestinationPage = R.Pages[DestinationPageIndex];
      if (S.BRL == "pre-paginated" && DestinationPage.Item.SpreadPair) {
        if (
          S.SLA == "horizontal" &&
          R.Stage[C.L_SIZE_L] > DestinationPage.Spread["offset" + C.L_SIZE_L]
        ) {
          if (Par.Distance < 0 && DestinationPage.IndexInSpread == 0)
            DestinationPage = DestinationPage.Spread.Pages[1];
          if (Par.Distance > 0 && DestinationPage.IndexInSpread == 1)
            DestinationPage = DestinationPage.Spread.Pages[0];
        }
      }
...
R.scrollBy = (Par) =>
  new Promise((resolve, reject) => {
    ...
    switch (S.SLD) {
      case "ttb":
        ScrollTarget.Y = R.Main.scrollTop + R.Stage.Height * Par.Distance;
        break;
      case "ltr":
        ScrollTarget.X = R.Main.scrollLeft + R.Stage.Width * Par.Distance;
        break;
      case "rtl":
        ScrollTarget.X = R.Main.scrollLeft + R.Stage.Width * Par.Distance * -1;
        break;
    }
    sML
      .scrollTo(ScrollTarget, {
  ...
  }).then(() => E.dispatch("bibi:scrolled-by", Par));
...
I.Utilities = {
  create: () => {
    const Utilities = (I.Utilities = I.setToggleAction(
      {
        openGracefuly: () =>
          R.Moving || R.Breaking || Utilities.UIState == "active"
            ? false
            : Utilities.open(),
        closeGracefuly: () =>
          R.Moving || R.Breaking || Utilities.UIState == "default"
            ? false
            : Utilities.close(),
        toggleGracefuly: () =>
          R.Moving || R.Breaking ? false : Utilities.toggle(),
      },
      {
    ...
    E.add("bibi:commands:toggle-utilities", () =>
...
I.ScrollObserver = {
  create: () => {
    const ScrollObserver = (I.ScrollObserver = {
      ...
      Count: 0,
      onScroll: (Eve) => {
        if (R.LayingOut || !L.Opened) return;
        if (!ScrollObserver.Scrolling) {
          ScrollObserver.Scrolling = true;
          O.HTML.classList.add("scrolling");
        }
        E.dispatch("bibi:is-scrolling");
        ScrollObserver.History.unshift(
          Math.ceil(R.Main["scroll" + C.L_OOBL_L])
        ); // Android Chrome returns scrollLeft/Top value of an element with slightly less float than actual.
      ...
      breakCurrentScrolling: () => {
        try {
          ...
          setTimeout(() => (R.Breaking = false), 333);
        } catch (Err) {
          R.Breaking = false;
        }
      },
      forceStopScrolling: () => {
        ...
        (R.Main.style.overflow = "hidden"),
          (R.Main.scrollLeft = R.Main.scrollLeft),
          (R.Main.scrollTop = R.Main.scrollTop),
          (R.Main.style.overflow = "");
      },
    });
...
I.PageObserver = {
  create: () => {
    const PageObserver = (I.PageObserver = {
      ...
      IntersectingPages: [],
      PagesToBeObserved: [],
      observePageIntersection: (Page) =>
        !PageObserver.PagesToBeObserved.includes(Page)
          ? PageObserver.PagesToBeObserved.push(Page)
          : PageObserver.PagesToBeObserved.length,
      unobservePageIntersection: (Page) =>
        (PageObserver.PagesToBeObserved = PageObserver.PagesToBeObserved.filter(
          (PageToBeObserved) => PageToBeObserved != Page
        )).length,
      observeIntersection: () => {
        const Glasses = new IntersectionObserver(
          (Ents) =>
            Ents.forEach((Ent) => {
              ...
              if (Ent.isIntersecting) {
                ...
                if (PageObserver.IntersectingPages.includes(Page)) {
                  IntersectionChanging = true;
                  PageObserver.IntersectingPages = PageObserver.IntersectingPages.filter(
                    (IntersectingPage) => IntersectingPage != Page
                  );
                }
              }
              if (IntersectionChanging) {
                if (PageObserver.IntersectingPages.length)
                  PageObserver.IntersectingPages.sort(
                    (A, B) => A.Index - B.Index
                  );
                E.dispatch(
                  "bibi:changes-intersection",
                  PageObserver.IntersectingPages
                );
        ...
        PageObserver.unobservePageIntersection = (Page) =>
          Glasses.unobserve(Page);
        PageObserver.PagesToBeObserved.forEach((PageToBeObserved) =>
          Glasses.observe(PageToBeObserved)
        );
      ...
      getFrame: () => {
        ...
        Frame[C.L_OOBL_L == "Top" ? "Bottom" : "Right"] =
          Frame[C.L_OOBL_L] + Frame.Length;
        ...
        return {
          Before: Frame[C.L_BASE_B],
          After: Frame[C.L_BASE_A],
          Length: Frame.Length,
        };
      },
      getCandidatePageList: () => {
        const QSW = Math.ceil((R.Stage.Width - 1) / 4),
          QSH = Math.ceil((R.Stage.Height - 1) / 4),
          CheckRoute = [5, 6, 4, 2, 8]; // 4x4 matrix
        for (let l = CheckRoute.length, i = 0; i < l; i++) {
          const CheckPoint = CheckRoute[i];
          const Ele = document.elementFromPoint(
            QSW * (CheckPoint % 3 || 3),
            QSH * Math.ceil(CheckPoint / 3)
          );
          if (Ele) {
            if (Ele.IndexInItem) return [Ele];
            // Page
            else if (Ele.Pages) return Ele.Pages;
            // Item or Spread
            else if (Ele.Inside) return Ele.Inside.Pages; // ItemBox or SpreadBox
          }
        }
        return PageObserver.IntersectingPages.length
          ? PageObserver.IntersectingPages
          : [];
      },
      getList: () => {
        let List = [],
          List_SpreadContained = [];
        const PageList = PageObserver.getCandidatePageList();
        if (!PageList.length || typeof PageList[0].Index != "number")
          return null;
        const FirstIndex = sML.limitMin(PageList[0].Index - 2, 0);
        const LastIndex = sML.limitMax(
          PageList[PageList.length - 1].Index + 2,
          R.Pages.length - 1
        );
        for (
          let BiggestPageIntersectionRatio = 0, i = FirstIndex;
          i <= LastIndex;
          i++
        ) {
          const Page = R.Pages[i];
          const PageIntersectionStatus = PageObserver.getIntersectionStatus(
            Page,
            "WithDetail"
          );
          if (PageIntersectionStatus.Ratio < BiggestPageIntersectionRatio) {
            ...
            const CurrentEntry = {
              Page: Page,
              PageIntersectionStatus: PageIntersectionStatus,
            };
            if (List.length) {
              const Prev = List[List.length - 1];
              if (Prev.Page.Item == Page.Item)
                CurrentEntry.ItemIntersectionStatus =
                  Prev.ItemIntersectionStatus;
              if (Prev.Page.Spread == Page.Spread)
                CurrentEntry.SpreadIntersectionStatus =
                  Prev.SpreadIntersectionStatus;
            }
            if (!CurrentEntry.ItemIntersectionStatus)
              CurrentEntry.ItemIntersectionStatus = PageObserver.getIntersectionStatus(
                Page.Item.Box
              ); // Item is scaled.
            if (!CurrentEntry.SpreadIntersectionStatus)
              CurrentEntry.SpreadIntersectionStatus = PageObserver.getIntersectionStatus(
                Page.Spread
              ); // SpreadBox has margin.
            if (CurrentEntry.SpreadIntersectionStatus.Ratio == 1)
              List_SpreadContained.push(CurrentEntry);
            if (PageIntersectionStatus.Ratio > BiggestPageIntersectionRatio)
              (List = [CurrentEntry]),
                (BiggestPageIntersectionRatio = PageIntersectionStatus.Ratio);
            else List.push(CurrentEntry);
          }
        }
        return List_SpreadContained.length
          ? List_SpreadContained
          : List.length
          ? List
          : null;
      ...
      getIntersectionStatus: (Ele, WithDetail) => {
        const Coord = sML.getCoord(Ele),
          _D = C.L_AXIS_D;
        const LengthInside =
          Math.min(
            PageObserver.Current.Frame.After * _D,
            Coord[C.L_BASE_A] * _D
          ) -
          Math.max(
            PageObserver.Current.Frame.Before * _D,
            Coord[C.L_BASE_B] * _D
          );
        const Ratio =
          LengthInside <= 0 || !Coord[C.L_SIZE_L] || isNaN(LengthInside)
            ? 0
            : LengthInside / Coord[C.L_SIZE_L];
        ...
        if (Ratio <= 0) {
          ...
          if (Ratio >= 1) {
            ...
            const FC_B = PageObserver.Current.Frame.Before * _D,
              FC_A = PageObserver.Current.Frame.After * _D;
            const PC_B = Coord[C.L_BASE_B] * _D,
              PC_A = Coord[C.L_BASE_A] * _D;
            ...
            if (R.Main["offset" + L] < Coord[C.L_SIZE_L])
              IntersectionStatus.Oversized = true;
          }
        }
      ...
      classify: () => {
        const CurrentElements = [],
          PastCurrentElements = R.Main.Book.querySelectorAll(".current");
        PageObserver.Current.List.forEach((CurrentEntry) => {
          const Page = CurrentEntry.Page,
            ItemBox = Page.Item.Box,
            SpreadBox = Page.Spread.Box;
          if (!CurrentElements.includes(SpreadBox))
            SpreadBox.classList.add("current"), CurrentElements.push(SpreadBox);
          if (!CurrentElements.includes(ItemBox))
            ItemBox.classList.add("current"), CurrentElements.push(ItemBox);
      ...
      observePageMove: () => {
        E.bind("bibi:scrolled", () => {
          const CS = PageObserver.Current.List[0],
            CE = PageObserver.Current.List.slice(-1)[0],
            CSP = CS.Page,
            CEP = CE.Page,
            CSPIS = CS.PageIntersectionStatus,
            CEPIS = CE.PageIntersectionStatus;
          const PS = PageObserver.Past.List[0],
            PE = PageObserver.Past.List.slice(-1)[0],
            PSP = PS.Page,
            PEP = PE.Page;
          const FPI = 0,
            LPI = R.Pages.length - 1;
          let Flipped = false,
            AtTheBeginning = false,
            AtTheEnd = false;
          if (CSP != PSP || CEP != PEP) {
            Flipped = true;
            if (CSP.Index == FPI && (CSPIS.Contained || CSPIS.Headed))
              AtTheBeginning = true;
            if (CEP.Index == LPI && (CEPIS.Contained || CEPIS.Footed))
              AtTheEnd = true;
          } else {
            const PSPIS = PS.PageIntersectionStatus,
              PEPIS = PE.PageIntersectionStatus;
            if (
              CSP.Index == FPI &&
              (CSPIS.Contained || CSPIS.Headed) &&
              !(PSPIS.Contained || PSPIS.Headed)
            )
              AtTheBeginning = true;
            if (
              CEP.Index == LPI &&
              (CEPIS.Contained || CEPIS.Footed) &&
              !(PEPIS.Contained || PEPIS.Footed)
            )
              AtTheEnd = true;
          }
          const ReturnValue = {
            Past: PageObserver.Past,
            Current: PageObserver.Current,
          };
          if (Flipped) E.dispatch("bibi:flipped", ReturnValue);
          if (AtTheBeginning)
            E.dispatch("bibi:got-to-the-beginning", ReturnValue);
      ...
      getIIPP: (Par = {}) => {
        const Page =
          Par.Page && typeof Par.Page.IndexInItem == "number"
            ? Par.Page
            : PageObserver.Current.Pages[0] ||
              (Par.Item && Par.Item.IndexInSpine
                ? Par.Item.Pages[0]
                : R.Pages[0]);
      ...
      getP: (Par = {}) => {
        ...
        if (
          Par.Element &&
          Par.Element.ownerDocument &&
          Par.Element.ownerDocument.body.Item
        ) {
          Item = Par.Element.ownerDocument.body.Item;
          Element = Par.Element;
        } else {
          if (B.Package.Metadata["rendition:layout"] == "pre-paginated")
            return {
              P: String(
                (PageObserver.Current.Pages[0] || R.Pages[0]).Item.Index + 1
              ),
            };
          if (!Par.Page || typeof Par.Page.IndexInItem != "number")
            Par.Page = null;
          const Page = Par.Page
            ? Par.Page
            ...
            ? PageObserver.Current.Pages[0]
            : PageObserver.IntersectingPages.filter(
                (ISP) =>
                  R.Stage[C.L_SIZE_L] *
                    PageObserver.getIntersectionStatus(ISP).Ratio >
                  3
              )[0];
          if (!Page) return "";
          Item = Page.Item;
          ...
          const PageArea = {
            Left: PageCoord.X,
            Right: PageCoord.X + Page.offsetWidth,
            Top: PageCoord.Y,
            Bottom: PageCoord.Y + Page.offsetHeight,
          };
          ...
          const HTMLArea = {
            Left: ItemCoord.X + S["item-padding-left"],
            Top: ItemCoord.Y + S["item-padding-top"],
          };
          (HTMLArea.Right = HTMLArea.Left + Item.HTML.offsetWidth),
            (HTMLArea.Bottom = HTMLArea.Top + Item.HTML.offsetHeight);
          const ViewArea = {
            Left: R.Main.scrollLeft,
            Right: R.Main.scrollLeft + R.Stage.Width,
            Top: R.Main.scrollTop,
            Bottom: R.Main.scrollTop + R.Stage.Height,
          };
          const PagedHTMLArea = Par.Page
            ? {
                Left: Math.max(PageArea.Left, HTMLArea.Left) - HTMLArea.Left,
                Right: Math.min(PageArea.Right, HTMLArea.Right) - HTMLArea.Left,
                Top: Math.max(PageArea.Top, HTMLArea.Top) - HTMLArea.Top,
                Bottom:
                  Math.min(PageArea.Bottom, HTMLArea.Bottom) - HTMLArea.Top,
              }
            : {
                Left:
                  Math.max(PageArea.Left, HTMLArea.Left, ViewArea.Left) -
                  HTMLArea.Left,
                Right:
                  Math.min(PageArea.Right, HTMLArea.Right, ViewArea.Right) -
                  HTMLArea.Left,
                Top:
                  Math.max(PageArea.Top, HTMLArea.Top, ViewArea.Top) -
                  HTMLArea.Top,
                Bottom:
                  Math.min(PageArea.Bottom, HTMLArea.Bottom, ViewArea.Bottom) -
                  HTMLArea.Top,
              };
          ...
          let LPM = 1,
            BPM = 1,
            getFirstElement = (l, b) =>
              Item.contentDocument.elementFromPoint(l, b);
          if (S.ARD == "rtl") {
            LPM *= -1;
          } else if (S.ARD == "ttb") {
            if (S.PPD == "rtl") BPM *= -1;
            getFirstElement = (l, b) =>
              Item.contentDocument.elementFromPoint(b, l);
          }
          if (Par.TagNames)
            Par.TagNames = !Array.isArray(Par.TagNames)
              ? null
              : Par.TagNames.map((TN) =>
                  typeof TN != "string"
                    ? ""
                    : TN.replace(/\s+/, "").toLowerCase()
                ).filter((TN) => (TN ? true : false));
          if (!Par.TagNames || !Par.TagNames.length) delete Par.TagNames;
          __: for (
            let i = 0,
              l = PagedHTMLArea[C.A_BASE_B],
              lp = Math.round(
                (PagedHTMLArea[C.A_BASE_A] - PagedHTMLArea[C.A_BASE_B]) / 9
              );
            l * LPM < PagedHTMLArea[C.A_BASE_A] * LPM;
            l += lp
          ) {
            for (
              let j = 0,
                b = PagedHTMLArea[C.A_BASE_S],
                bp = Math.round(
                  (PagedHTMLArea[C.A_BASE_E] - PagedHTMLArea[C.A_BASE_S]) / 3
                );
              b * BPM < PagedHTMLArea[C.A_BASE_E] * BPM;
              b += bp
            ) {
              const Ele = getFirstElement(l, b);
              if (Par.TagNames) {
                if (Par.TagNames.includes(Ele.tagName.toLowerCase()))
                  Element = Ele;
          ...
          if (!Element)
            return Page.IndexInItem < Item.Pages.length - 1
              ? PageObserver.getP({
                  Page: Item.Pages[Page.IndexInItem + 1],
                  TagNames: Par.TagNames,
                })
              : Item.Index < R.Items.length - 1
              ? Item.Index + 2
              : "foot";
        }
        const Steps = [];
      ...
      getTurningOriginItem: (Dir = 1) => {
        const List = PageObserver.Current.List.length
          ? PageObserver.Current.List
          : PageObserver.IntersectingPages.length
          ? PageObserver.IntersectingPages
          : null;
        try {
          return (Dir > 0 ? List[0] : List[List.length - 1]).Page.Item;
        } catch (Err) {}
        return null;
      },
      turnItems: (Opt) => {
        ...
        const Dir =
          I.ScrollObserver.History.length > 1 &&
          I.ScrollObserver.History[1] * C.L_AXIS_D >
            I.ScrollObserver.History[0] * C.L_AXIS_D
            ? -1
            : 1;
        const Origin = Opt.Origin || PageObserver.getTurningOriginItem(Dir);
        if (!Origin) return;
        const MUST = [Origin],
          NEW_ = [],
          KEEP = []; // const ___X = [];
        const Next = R.Items[Origin.Index + Dir];
        if (Next) MUST.push(Next);
        const Prev = R.Items[Origin.Index - Dir];
        if (Prev) MUST.push(Prev);
        MUST.forEach((Item) => {
          if (Item.Turned != "Up")
            (PageObserver.TurningItems_FaceUp.includes(Item)
              ? KEEP
              : NEW_
            ).push(Item);
        });
        ...
        while (
          ++i < 9 /* && x < 2*/ &&
          KEEP.length + NEW_.length < PageObserver.MaxTurning
        ) {
          const Item = R.Items[Origin.Index + i * Dir];
          if (!Item) break;
        ...
        PageObserver.TurningItems_FaceUp.forEach((Item) => {
          // use `forEach`, not `for`.
          if (KEEP.includes(Item)) return;
          if (KEEP.length + NEW_.length < PageObserver.MaxTurning)
            return KEEP.push(Item);
          clearTimeout(Item.Timer_Turn);
          PageObserver.turnItem(Item, { Down: true }); // ___X.push(Item);
        });
        NEW_.forEach((Item, i) =>
          PageObserver.turnItem(Item, {
            Up: true,
            Delay: (MUST.includes(Item) ? 99 : 999) * i,
          })
        );
        /*
            console.log('--------');
      ...
      turnItem: (Item, Opt) =>
        new Promise((resolve) => {
          ...
          if (Opt.Up) {
            if (PageObserver.TurningItems_FaceUp.includes(Item))
              return resolve();
            PageObserver.TurningItems_FaceUp.push(Item),
              (PageObserver.TurningItems_FaceDown = PageObserver.TurningItems_FaceDown.filter(
                (_) => _ != Item
              ));
          } else {
            if (PageObserver.TurningItems_FaceDown.includes(Item))
              return resolve();
            PageObserver.TurningItems_FaceDown.push(Item),
              (PageObserver.TurningItems_FaceUp = PageObserver.TurningItems_FaceUp.filter(
                (_) => _ != Item
              ));
    ...
    E.bind("bibi:laid-out-for-the-first-time", (LayoutOption) => {
      PageObserver.IntersectingPages = [
        R.Spreads[LayoutOption.TargetSpreadIndex].Pages[0],
      ];
    ...
    const ResizeObserver = (I.ResizeObserver = {
      ...
      onResize: (Eve) => {
        ...
        if (!ResizeObserver.Resizing) {
          ...
          ResizeObserver.TargetPageAfterResizing = I.PageObserver.Current
            .List[0]
            ? I.PageObserver.Current.List[0].Page
            : null;
        ...
        ResizeObserver.Timer_onResizeEnd = setTimeout(
          () => {
            R.updateOrientation();
            const Page =
              ResizeObserver.TargetPageAfterResizing ||
              (I.PageObserver.Current.List[0]
                ? I.PageObserver.Current.List[0].Page
                : null);
            R.layOutBook({
              ...
              Destination: Page
                ? {
                    ItemIndex: Page.Item.Index,
                    PageProgressInItem:
                      Page.IndexInItem / Page.Item.Pages.length,
                  }
                : null,
            }).then(() => {
...
I.TouchObserver = {
  create: () => {
    const TouchObserver = (I.TouchObserver = {
      observeElementHover: (Ele) => {
        if (!Ele.BibiHoverObserver) {
          ...
          Ele.addEventListener(E["pointerover"], (Eve) =>
            Ele.BibiHoverObserver.onHover(Eve)
          );
          Ele.addEventListener(E["pointerout"], (Eve) =>
            Ele.BibiHoverObserver.onUnHover(Eve)
          );
        }
        return Ele;
      },
      setElementHoverActions: (Ele) => {
        E.add(Ele, "bibi:hovered", (Eve) => {
          if (Ele.Hover || (Ele.isAvailable && !Ele.isAvailable(Eve)))
            return Ele;
          Ele.Hover = true;
        ...
        if (!Ele.BibiTapObserver) {
          ...
          Ele.BibiTapObserver = {
            ...
            onPointerDown: function (Eve) {
              if (
                (typeof Eve.buttons == "number" && Eve.buttons !== 1) ||
                Eve.ctrlKey
              )
                return true;
              this.care(Eve);
            ...
            onPointerUp: function (Eve) {
              ...
              if (UpTime - this.Touching.Time < TimeLimit.D2U) {
                ...
                if (
                  Math.abs(TouchEndCoord.X - this.Touching.Coord.X) < 3 &&
                  Math.abs(TouchEndCoord.Y - this.Touching.Coord.Y) < 3
                ) {
                  let SDT = 0;
                  const Floating = {
                    Time: UpTime,
                  ...
                  if (!this.Floating) {
                    ...
                    this.Floating = Object.assign(Floating, {
                  ...
                  this.Timer_fireTap = setTimeout(() => {
                    switch (SDT) {
                      case 1:
                        return Ele.BibiTapObserver.onTap(Eve);
                      case 2:
                        return Ele.BibiTapObserver.onDoubleTap(Eve);
                      case 3:
                        return Ele.BibiTapObserver.onTripleTap(Eve);
                    }
                  }, TimeLimit.U2D);
                } else {
                  delete this.Floating;
          ...
          Ele.addEventListener(E["pointerdown"], (Eve) =>
            Ele.BibiTapObserver.onPointerDown(Eve)
          );
          Ele.addEventListener(E["pointerup"], (Eve) =>
            Ele.BibiTapObserver.onPointerUp(Eve)
          );
        }
        return Ele;
      },
      setElementTapActions: (Ele) => {
          ...
          switch (Ele.Type) {
            case "toggle":
              return (Eve) => {
                ...
                I.setUIState(
                  Ele,
                  Ele.UIState == "default" ? "active" : "default"
                );
              };
            case "radio":
              return (Eve) => {
        ...
        E.add(Ele, "bibi:tapped", (Eve) => {
          if (
            (Ele.isAvailable && !Ele.isAvailable(Eve)) ||
            Ele.UIState == "disabled" ||
            (Ele.UIState == "active" && Ele.Type == "radio")
          )
            return Ele;
          onTap(Eve);
          if (Ele.hideHelp) Ele.hideHelp();
      ...
      activateHTML: (HTML) => {
        ...
        E.add(HTML, "bibi:doubletapped", (Eve) =>
          E.dispatch("bibi:doubletapped", Eve)
        ); //HTML.ownerDocument.addEventListener('dblclick', Eve => { Eve.preventDefault(); Eve.stopPropagation(); return false; });
        E.add(HTML, "bibi:tripletapped", (Eve) =>
          E.dispatch("bibi:tripletapped", Eve)
        );
        E.add(
          HTML,
          TOPENs[0],
          (Eve) => E.dispatch("bibi:downed-pointer", Eve),
          E.Cpt1Psv0
        );
        E.add(
          HTML,
          TOPENs[1],
          (Eve) => E.dispatch("bibi:upped-pointer", Eve),
          E.Cpt1Psv0
        );
        E.add(
          HTML,
          TOPENs[2],
          (Eve) => {
            const CC = O.getBibiEventCoord(Eve),
              PC = TouchObserver.PreviousPointerCoord;
            E.dispatch(
              PC.X != CC.X || PC.Y != CC.Y
                ? "bibi:moved-pointer"
    ...
    const checkTapAvailability = (BibiEvent) => {
      switch (S.RVM) {
        case "horizontal":
          if (BibiEvent.Coord.Y > window.innerHeight - O.Scrollbars.Height)
            return false;
          else break;
        case "vertical":
          if (BibiEvent.Coord.X > window.innerWidth - O.Scrollbars.Width)
            return false;
          else break;
      }
      if (BibiEvent.Target.ownerDocument) {
        ...
        if (
          I.Slider.ownerDocument &&
          (BibiEvent.Target == I.Slider || I.Slider.contains(BibiEvent.Target))
        )
          return false;
      }
      return true;
    ...
    E.bind("bibi:postprocessed-item", (Item) =>
      TouchObserver.activateHTML(Item.HTML)
    );
    E.add("bibi:tapped", (Eve) => {
      ...
      return BibiEvent.Division.X == "center" &&
        BibiEvent.Division.Y == "middle"
        ? E.dispatch("bibi:tapped-center", Eve)
        : E.dispatch("bibi:tapped-not-center", Eve);
    });
    E.add("bibi:doubletapped", (Eve) => {
      if (I.isPointerStealth() || !L.Opened) return false;
      if (I.OpenedSubpanel) return I.OpenedSubpanel.close() && false;
      const BibiEvent = O.getBibiEvent(Eve);
      if (!checkTapAvailability(BibiEvent)) return false;
      return BibiEvent.Division.X == "center" &&
        BibiEvent.Division.Y == "middle"
        ? E.dispatch("bibi:doubletapped-center", Eve)
        : E.dispatch("bibi:doubletapped-not-center", Eve);
    });
...
I.FlickObserver = {
  create: () => {
    const FlickObserver = (I.FlickObserver = {
      ...
      getDegree: (_) =>
        ((Deg) => (Deg < 0 ? Deg + 360 : Deg))(
          (Math.atan2(_.Y * -1, _.X) * 180) / Math.PI
        ),
      onTouchStart: (Eve) => {
        if (!L.Opened) return;
      ...
      onTouchMove: (Eve) => {
        ...
        if (FlickObserver.StartedAt) {
          if (!FlickObserver.Moving) {
            const TimeFromTouchStarted =
              Eve.timeStamp - FlickObserver.StartedAt.TimeStamp;
            if (
              !O.TouchOS &&
              (Eve.type == "mousemove" || Eve.pointerType == "mouse")
            ) {
              if (TimeFromTouchStarted < 234) return FlickObserver.cancel();
            } else {
              if (TimeFromTouchStarted > 234) return FlickObserver.cancel();
            }
          ...
          const EventCoord = O.getBibiEventCoord(Eve);
          const Passage = {
            X: EventCoord.X - FlickObserver.StartedAt.X,
            Y: EventCoord.Y - FlickObserver.StartedAt.Y,
          };
          if (++FlickObserver.Moving <= 3) {
            const Deg = FlickObserver.getDegree(Passage);
            FlickObserver.StartedAt.LaunchingAxis =
              315 <= Deg || Deg <= 45 || (135 <= Deg && Deg <= 225)
                ? "X"
                : (45 < Deg && Deg < 135) || (225 < Deg && Deg < 315)
                ? "Y"
                : "";
          }
          if (FlickObserver.StartedAt.LaunchingAxis == C.A_AXIS_B) {
            ...
            // Natural
            if (S.RVM != "paged" && Eve.type == "touchmove")
              return FlickObserver.cancel();
            if (
              S["content-draggable"][S.RVM == "paged" ? 0 : 1] &&
              I.isScrollable()
            )
              R.Main["scroll" + C.L_OOBL_L] =
                FlickObserver.StartedAt["Scroll" + C.L_OOBL_L] +
                Passage[C.L_AXIS_L] * -1;
          }
          Eve.preventDefault();
          ...
          FlickObserver.LastEvent = Eve;
          if (
            EventCoord[C.A_AXIS_L] <= 0 ||
            EventCoord[C.A_AXIS_L] >= R.Stage[C.A_SIZE_L] ||
            EventCoord[C.A_AXIS_B] <= 0 ||
            EventCoord[C.A_AXIS_B] >= R.Stage[C.A_SIZE_B]
          )
            return FlickObserver.onTouchEnd(Eve, { Swipe: true });
        }
      },
      onTouchEnd: (Eve, Opt) => {
        ...
        if (FlickObserver.StartedAt) {
          ...
          if (FlickObserver.StartedAt.Item)
            FlickObserver.StartedAt.Item.HTML.classList.remove(
              "bibi-flick-hot"
            );
          if (!I.Loupe.Transforming) {
            if (
              FlickObserver.StartedAt.LaunchingAxis == C.A_AXIS_B &&
              Math.abs(Passage[C.A_AXIS_B] / 100) >= 1
            ) {
            ...
            if (!cb && (Math.abs(Passage.X) >= 3 || Math.abs(Passage.Y) >= 3)) {
              ...
              Eve.preventDefault();
              Par.Speed =
                Math.sqrt(Math.pow(Passage.X, 2) + Math.pow(Passage.Y, 2)) /
                (Eve.timeStamp - FlickObserver.StartedAt.TimeStamp);
              Par.Deg = FlickObserver.getDegree(Passage);
              if (
                O.getViewportZooming() <= 1 &&
                Eve.timeStamp - FlickObserver.StartedAt.TimeStamp <= 300
              ) {
                Par.OriginList = FlickObserver.StartedAt.OriginList;
                cb =
                  Opt && Opt.Swipe
                    ? FlickObserver.onSwipe
                    : FlickObserver.onFlick;
      ...
      onFlick: (Eve, Par) => {
        ...
        if (!Dist) {
          ...
          const PageIndex =
            Dist > 0
              ? Par.OriginList.slice(-1)[0].Page.Index
              : Par.OriginList[0].Page.Index;
          return R.focusOn({
            Destination: {
              Page: R.Pages[PageIndex + Dist]
                ? R.Pages[PageIndex + Dist]
                : R.Pages[PageIndex],
            },
            Duration: !I.isScrollable()
              ? 0
              : S.RVM != "paged" || S["content-draggable"][0]
              ? 123
              : 0,
          });
        } else {
          ...
          return R.scrollBy({
            Distance:
              ...
              (Par.Speed
                ? (sML.limitMinMax(
                    Math.round(Par.Speed * 100) * 0.08,
                    0.33,
                    10
                  ) *
                    333) /
                  (S.SLD == "ttb" ? R.Stage.Height : R.Stage.Width)
                : 1),
            Duration: 1234,
            Cancelable: true,
      ...
      onPanRelease: (Eve, Par) => {
        if (S.RVM != "paged" || !S["content-draggable"][0])
          return Promise.resolve(); // Only for Paged View ====
        const Deg = Par.Deg;
        const Dir =
          270 < Deg || Deg < 90
            ? "left" /* to right */
        ...
        return R.focusOn({
          Destination: {
            Page:
              Dist >= 0 ? CurrentList.slice(-1)[0].Page : CurrentList[0].Page,
          },
          Duration: !I.isScrollable() ? 0 : S["content-draggable"][0] ? 123 : 0,
        });
      },
      getOrthogonalTouchMoveFunction: () => {
        switch (I.orthogonal("touch-moves")) {
          case "switch":
            if (I.AxisSwitcher) return I.AxisSwitcher.switchAxis;
            break;
          case "utilities":
            return I.Utilities.toggleGracefuly;
            break;
        }
      },
      ...
      activateElement: (Ele) => {
        ...
        Ele.addEventListener(
          E["pointerdown"],
          FlickObserver.onTouchStart,
          E.Cpt1Psv0
        );
        const CNPf = FlickObserver.getCNPf(Ele);
        /**/ Ele.ownerDocument.documentElement.classList.add(
          CNPf + "flick-active"
        );
        if (I.isScrollable())
          Ele.ownerDocument.documentElement.classList.add(
      ...
      deactivateElement: (Ele) => {
        ...
        const CNPf = FlickObserver.getCNPf(Ele);
        Ele.ownerDocument.documentElement.classList.remove(
          CNPf + "flick-active"
        );
        Ele.ownerDocument.documentElement.classList.remove(
          CNPf + "flick-scrollable"
    ...
    E.add("bibi:loaded-item", (Item) =>
      FlickObserver.activateElement(Item.HTML)
    );
  ...
  create: () => {
    const WheelObserver = (I.WheelObserver = {
      ...
      reserveResetWith: (fn) => {
        ...
        WheelObserver.Timer_resetWheeling = setTimeout(
          WheelObserver.reset,
          234
        );
      },
      ...
      heat: () => {
        ...
        WheelObserver.Timer_coolDown = setTimeout(
          () => (WheelObserver.Hot = false),
          234
        );
      },
      onWheel: (Eve) => {
        ...
        const WA /* WheelAxis */ =
          Math.abs(Eve.deltaX) > Math.abs(Eve.deltaY) ? "X" : "Y";
        const CW /* CurrentWheel */ = {},
          Ws = WheelObserver.Wheels,
          Wl = Ws.length;
        ...
        ////////
        else if (CW.Distance != Ws[Wl - 1].Distance)
          (CW.Accel = 1),
            (CW.Wheeled =
              Wl >= 3 &&
              Ws[Wl - 2].Distance != CW.Distance &&
              Ws[Wl - 3].Distance != CW.Distance
                ? "reverse"
                : "");
        else if (CW.Delta > Ws[Wl - 1].Delta)
          (CW.Accel = 1),
            (CW.Wheeled =
              Wl >= 3 &&
              Ws[Wl - 1].Accel == -1 &&
          ...
          (CW.Accel = -1), (CW.Wheeled = "");
        else (CW.Accel = Ws[Wl - 1].Accel), (CW.Wheeled = "");
        WheelObserver.reserveResetWith(() => {
          Ws.push(CW);
          if (Wl > 3) Ws.shift();
          WheelObserver.Progress =
            (WheelObserver.TotalDelta += Eve["delta" + WA]) / 3 / 100;
        });
        const ToDo =
          WA != C.A_AXIS_L
            ? I.orthogonal("wheelings")
            : S.RVM == "paged"
            ? "move"
            : WheelObserver.OverlaidUIs.filter((OUI) =>
                OUI.contains(Eve.target)
              ).length
            ? "simulate"
            : "";
        if (!ToDo) return;
        ...
        switch (ToDo) {
          case "simulate":
            return WheelObserver.scrollNatural(Eve, WA);
          case "across":
            return WheelObserver.scrollAcross(Eve, WA);
          case "move":
            return WheelObserver.move(CW);
          case "utilities":
            return WheelObserver.toggleUtilities(CW);
          case "switch":
            return WheelObserver.switchAxis(CW);
        }
      },
      scrollNatural: (Eve, Axis) => {
        switch (Axis) {
          case "X":
            R.Main.scrollLeft += Eve.deltaX;
            break;
          case "Y":
            R.Main.scrollTop += Eve.deltaY;
            break;
        }
      },
      scrollAcross: (Eve, Axis) => {
        switch (Axis) {
          case "X":
            R.Main.scrollTop += Eve.deltaX;
            break;
          case "Y":
            R.Main.scrollLeft += Eve.deltaY * (S.ARD == "rtl" ? -1 : 1);
            break;
        }
      },
      move: (CW) => {
    ...
    document.addEventListener(
      "wheel",
      (Eve) => E.dispatch("bibi:is-wheeling", Eve),
      E.Cpt1Psv0
    );
    E.add("bibi:loaded-item", (Item) =>
      Item.contentDocument.addEventListener(
        "wheel",
        (Eve) => E.dispatch("bibi:is-wheeling", Eve),
        E.Cpt1Psv0
      )
...
I.PinchObserver = {
  create: () => {
    const PinchObserver = (I.PinchObserver = {
      Pinching: 0,
      getEventCoords: (Eve) => {
        const T0 = Eve.touches[0],
          T1 = Eve.touches[1],
          Doc = Eve.target.ownerDocument;
        const T0CoordInViewport = { X: T0.screenX, Y: T0.screenY };
        const T1CoordInViewport = { X: T1.screenX, Y: T1.screenY };
        return {
          Center: {
            X: (T0CoordInViewport.X + T1CoordInViewport.X) / 2,
            Y: (T0CoordInViewport.Y + T1CoordInViewport.Y) / 2,
          },
          Distance: Math.sqrt(
            Math.pow(T1.screenX - T0.screenX, 2) +
              Math.pow(T1.screenY - T0.screenY, 2)
          ),
        };
      },
      onTouchStart: (Eve) => {
      ...
      onTouchMove: (Eve) => {
        ...
        Eve.stopPropagation();
        const Ratio =
          PinchObserver.getEventCoords(Eve).Distance /
          PinchObserver.PinchStart.Coords.Distance;
        ...
        I.Loupe.scale(PinchObserver.PinchStart.Scale * Ratio, {
          Center: PinchObserver.PinchStart.Coords.Center,
          Stepless: true,
        });
        PinchObserver.Timer_TransitionRestore = setTimeout(
          () => sML.style(R.Main, { transition: "" }),
          234
        );
      },
      onTouchEnd: (Eve, Opt) => {
      ...
      activateElement: (Ele) => {
        ...
        Ele.addEventListener(
          "touchstart",
          PinchObserver.onTouchStart,
          E.Cpt1Psv0
        );
        Ele.addEventListener(
          "touchmove",
          PinchObserver.onTouchMove,
          E.Cpt1Psv0
        );
        Ele.addEventListener("touchend", PinchObserver.onTouchEnd, E.Cpt1Psv0);
        Ele.ownerDocument.documentElement.classList.add(
          PinchObserver.getCNPf(Ele) + "pinch-active"
        );
      },
      deactivateElement: (Ele) => {
        if (!Ele) return false;
    ...
    E.add("bibi:loaded-item", (Item) =>
      PinchObserver.activateElement(Item.HTML)
    );
  ...
  create: () => {
    ...
    const KeyObserver = (I.KeyObserver = {
      ...
      updateKeyCodes: (EventTypes, KeyCodesToUpdate) => {
        ...
        EventTypes.forEach(
          (EventType) =>
            (KeyObserver.KeyCodes[EventType] = sML.edit(
              KeyObserver.KeyCodes[EventType],
              KeyCodesToUpdate
            ))
        );
      },
      ...
      initializeKeyParameters: () => {
        ...
        for (const p in _)
          _[p.toUpperCase()] =
            _[p] == "head" ? "foot" : _[p] == "foot" ? "head" : _[p];
          ...
          switch (S.ARA) {
            case "horizontal":
              return Object.assign(
                { "Left Arrow": C.d2d("left"), "Right Arrow": C.d2d("right") },
                _O == "move"
                  ? {
                      "Up Arrow": C.d2d("top", 9),
                      "Down Arrow": C.d2d("bottom", 9),
                    }
                  : { "Up Arrow": _O, "Down Arrow": _O }
              );
            case "vertical":
              return Object.assign(
                { "Up Arrow": C.d2d("top"), "Down Arrow": C.d2d("bottom") },
                _O == "move"
                  ? {
                      "Left Arrow": C.d2d("left", 9),
                      "Right Arrow": C.d2d("right", 9),
                    }
                  : { "Left Arrow": _O, "Right Arrow": _O }
              );
          }
        })();
        for (const p in _)
          _[p.toUpperCase()] = _[p] == -1 ? "head" : _[p] == 1 ? "foot" : _[p];
      ...
      onKeyUp: (Eve) => {
        if (!KeyObserver.onEvent(Eve)) return false;
        if (
          KeyObserver.ActiveKeys[Eve.BibiKeyName] &&
          Date.now() - KeyObserver.ActiveKeys[Eve.BibiKeyName] < 300
        ) {
      ...
      observe: (Doc) => {
        ["keydown", "keyup", "keypress"].forEach((EventName) =>
          Doc.addEventListener(
            EventName,
            KeyObserver["onKey" + sML.capitalise(EventName.replace("key", ""))],
            false
          )
        );
      },
      onKeyTouch: (Eve) => {
        ...
        const KeyParameter =
          KeyObserver.KeyParameters[
            !Eve.shiftKey ? Eve.BibiKeyName : Eve.BibiKeyName.toUpperCase()
          ];
        ...
        switch (typeof KeyParameter) {
          case "number":
            if (I.Flipper.isAbleToFlip(KeyParameter)) {
              if (I.Arrows)
                E.dispatch(I.Arrows[KeyParameter], "bibi:tapped", Eve);
          ...
          case "string":
            switch (KeyParameter) {
              case "head":
              case "foot":
                return R.focusOn({ Destination: KeyParameter, Duration: 0 });
              case "utilities":
                return I.Utilities.toggleGracefuly();
              case "switch":
                return I.AxisSwitcher ? I.AxisSwitcher.switchAxis() : false;
            }
            break;
        }
      },
    ...
    E.add("bibi:postprocessed-item", (Item) =>
      Item.IsPlaceholder ? false : KeyObserver.observe(Item.contentDocument)
    );
    E.add("bibi:opened", () => {
      KeyObserver.initializeKeyParameters(),
        KeyObserver.updateKeyParameters(),
        E.add("bibi:changed-view", () => KeyObserver.updateKeyParameters());
      KeyObserver.observe(document);
      E.add(["bibi:touched-key", "bibi:is-holding-key"], (Eve) =>
        KeyObserver.onKeyTouch(Eve)
      );
    });
    ...
    E.add("bibi:opened", () => {
      E.add(
        ...
        (Eve) => {
          if (I.isPointerStealth()) return false;
          const BibiEvent = O.getBibiEvent(Eve);
          const Dir = I.Flipper.getDirection(BibiEvent.Division),
            Ortho = I.orthogonal("edges"),
            Dist = C.d2d(Dir, Ortho == "move");
          if (Dist) {
            if (I.Arrows && I.Arrows.areAvailable(BibiEvent))
              E.dispatch(I.Arrows[Dist], "bibi:tapped", Eve);
            if (I.Flipper.isAbleToFlip(Dist)) I.Flipper.flip(Dist);
          } else if (typeof C.DDD[Dir] == "string")
            switch (Ortho) {
              case "utilities":
                I.Utilities.toggleGracefuly();
                break;
              case "switch":
                if (I.AxisSwitcher) I.AxisSwitcher.switchAxis();
                break;
            }
        }
      );
        ...
        E.add("bibi:moved-pointer", (Eve) => {
          ...
          if (I.Arrows.areAvailable(BibiEvent)) {
            ...
            if (Dist && I.Flipper.isAbleToFlip(Dist)) {
              ...
              const HoveringHTML =
                BibiEvent.Target.ownerDocument.documentElement;
              if (EdgeObserver.HoveringHTML != HoveringHTML) {
                if (EdgeObserver.HoveringHTML)
                  EdgeObserver.HoveringHTML.removeAttribute("data-bibi-cursor");
                (EdgeObserver.HoveringHTML = HoveringHTML).setAttribute(
                  "data-bibi-cursor",
                  Dir
                );
              }
          ...
          if (EdgeObserver.Hovering) {
            ...
            if (I.Arrows)
              E.dispatch(
                [I.Arrows.Back, I.Arrows.Forward],
                "bibi:unhovered",
                Eve
              );
            if (EdgeObserver.HoveringHTML)
              EdgeObserver.HoveringHTML.removeAttribute("data-bibi-cursor"),
                (EdgeObserver.HoveringHTML = null);
          }
        });
        sML.forEach(O.Body.querySelectorAll("img"))((Img) =>
          Img.addEventListener(E["pointerdown"], O.preventDefault)
        );
      }
    });
    if (!O.TouchOS) {
      E.add("bibi:loaded-item", (Item) =>
        sML.forEach(Item.Body.querySelectorAll("img"))((Img) =>
          Img.addEventListener(E["pointerdown"], O.preventDefault)
        )
      );
    }
...
I.Notifier = {
  create: () => {
    const Notifier = (I.Notifier = O.Body.appendChild(
      sML.create("div", {
        id: "bibi-notifier",
        show: (Msg, Err) => {
          ...
          if (L.Opened && !Err)
            Notifier.addEventListener(
              O.TouchOS ? E["pointerdown"] : E["pointerover"],
              Notifier.hide
            );
        },
        hide: (Time = 0) => {
        ...
        note: (Msg, Time, Err) => {
          ...
          if (typeof Time == "undefined")
            Time = O.Busy && !L.Opened ? 8888 : 2222;
...
I.Veil = {
  create: () => {
    const Veil = (I.Veil = I.setToggleAction(
      O.Body.appendChild(sML.create("div", { id: "bibi-veil" })),
      {
        // Translate: 240, /* % */ // Rotate: -48, /* deg */ // Perspective: 240, /* px */
        onopened: () => (
          O.HTML.classList.add("veil-opened"), Veil.classList.remove("closed")
        ),
        onclosed: () => (
          Veil.classList.add("closed"), O.HTML.classList.remove("veil-opened")
        ),
      }
    ));
    ["touchstart", "pointerdown", "mousedown", "click"].forEach((EN) =>
      Veil.addEventListener(EN, (Eve) => Eve.stopPropagation(), E.Cpt0Psv0)
    );
    Veil.open();
    ...
    const PlayButton = (Veil.PlayButton = Veil.appendChild(
      sML.create("p", {
        ...
        play: (Eve) => (
          Eve.stopPropagation(), L.play(), E.dispatch("bibi:played:by-button")
        ),
        hide: () =>
          sML
            .style(PlayButton, { opacity: 0, cursor: "default" })
            .then((Eve) => Veil.removeChild(PlayButton)),
    ...
    Veil.byebye = (Msg = {}) => {
      Veil.innerHTML = "";
      Veil.ByeBye = Veil.appendChild(
        sML.create("p", { id: "bibi-veil-byebye" })
      );
      ["en", "ja"].forEach(
        (Lang) =>
          (Veil.ByeBye.innerHTML += `<span lang="${Lang}">${Msg[Lang]}</span>`)
      );
      O.HTML.classList.remove("welcome");
      Veil.open();
    ...
    Veil.Cover.Info = Veil.Cover.appendChild(
...
I.Catcher = {
  create: () => {
    if (S["book-data"] || S["book"] || !S["accept-local-file"]) return;
    const Catcher = (I.Catcher = O.Body.appendChild(
      sML.create("div", { id: "bibi-catcher" })
    ));
    Catcher.insertAdjacentHTML(
      "afterbegin",
      I.distillLabels.distillLanguage({
        default: [
    ...
    Catcher.title = Catcher.querySelector("span")
      .innerHTML.replace(/<br( ?\/)?>/g, "\n")
      .replace(/<[^>]+>/g, "")
      .trim();
    Catcher.Input = Catcher.appendChild(sML.create("input", { type: "file" }));
    if (
      !S["extract-if-necessary"].includes("*") &&
      S["extract-if-necessary"].length
    ) {
      const Accept = [];
    ...
    if (!O.TouchOS) {
      Catcher.addEventListener(
        ...
        (Eve) => {
          Eve.preventDefault();
          O.HTML.classList.add("dragenter");
        },
        1
      );
      Catcher.addEventListener(
      ...
      Catcher.addEventListener(
        ...
        (Eve) => {
          Eve.preventDefault();
          O.HTML.classList.remove("dragenter");
        },
        1
      );
      Catcher.addEventListener(
        ...
        (Eve) => {
          ...
          let FileData = {};
          try {
            FileData = Eve.dataTransfer.files[0];
          } catch (_) {}
          Bibi.getBookData.resolve({ BookData: FileData });
        },
        1
...
I.Menu = {
  create: () => {
    ...
    const Menu = (I.Menu = O.Body.appendChild(
      sML.create("div", { id: "bibi-menu" }, I.Menu)
    ));
    delete Menu.create;
    ...
    E.add("bibi:opens-utilities", (Opt) =>
      E.dispatch("bibi:commands:open-menu", Opt)
    );
    E.add("bibi:closes-utilities", (Opt) =>
      E.dispatch("bibi:commands:close-menu", Opt)
    );
    E.add(
      "bibi:opened",
      Menu.close
    ...
    if (!O.TouchOS)
      E.add("bibi:opened", () => {
        E.add("bibi:moved-pointer", (Eve) => {
          ...
          if (BibiEvent.Division.Y == "top") {
            //if(BibiEvent.Coord.Y < Menu.Height * 1.5) {
            E.dispatch(Menu, "bibi:hovered", Eve);
          } else if (Menu.Hover) {
            Menu.Timer_close = setTimeout(
              () => E.dispatch(Menu, "bibi:unhovered", Eve),
              123
            );
          }
        });
    ...
    });
    {
      ...
      sML.appendCSSRule(
        _Common + _M,
        "width: calc(100% - " + O.Scrollbars.Width + "px);"
      );
      sML.appendCSSRule(
        [
          _Common + ".panel-opened" + _M,
          _Common + ".subpanel-opened" + _M,
        ].join(", "),
        "padding-right: " + O.Scrollbars.Width + "px;"
      );
    }
...
I.Menu.Config = {
  create: () => {
    ...
    if (
      S["website-href"] &&
      /^https?:\/\/[^\/]+/.test(S["website-href"]) &&
      S["website-name-in-menu"]
    )
      Components.push("WebsiteLink");
    if (!S["remove-bibi-website-link"]) Components.push("BibiWebsiteLink");
    if (!Components.length) {
      delete I.Menu.Config;
      return;
    }
    const Config = (Menu.Config = sML.applyRtL(
      I.createSubpanel({ id: "bibi-subpanel_config" }),
      Menu.Config
    ));
    delete Config.create;
    const Opener = Config.bindOpener(
      Menu.R.addButtonGroup({ Sticky: true }).addButton({
        Type: "toggle",
        Labels: {
    ...
    else delete Config.ViewModeSection;
    if (
      Components.includes("NewWindowButton") ||
      Components.includes("FullscreenButton")
    )
      Config.WindowSection.create(Components);
    else delete Config.WindowSection;
    if (
      Components.includes("WebsiteLink") ||
      Components.includes("BibiWebsiteLink")
    )
      Config.LinkageSection.create(Components);
    else delete Config.LinkageSection;
...
I.Menu.Config.ViewModeSection = {
  create: () => {
    ...
    const /* SpreadShapes */ SSs = /* SpreadShape */ ((SS) => SS + SS + SS)(
        /* ItemShape */ ((IS) =>
          `<span class="bibi-shape bibi-shape-spread">${IS + IS}</span>`)(
          ...
          Buttons: [
            {
              Mode: "paged",
              Labels: {
                default: { default: `Spread / Page`, ja: `見開き／ページ` },
              },
            ...
            sML.edit(Button, {
              Notes: true,
              action: () =>
                R.changeView({ Mode: Button.Mode, NoNotification: true }),
            })
          ),
        },
        /*{
          ...
          Buttons: [
            {
              ...
              action: function () {
                ...
                if (S.RVM == "horizontal" || S.RVM == "vertical")
                  R.changeView({ Mode: S.RVM, Force: true });
                if (S["keep-settings"] && O.Biscuits)
                  O.Biscuits.memorize("Book", {
    ...
    E.add("bibi:updated-settings", () => {
      Section.ButtonGroups[0].Buttons.forEach((Button) =>
  ...
  create: (Components) => {
    ...
    if (Components.includes("FullscreenButton")) {
      Buttons.push({
        ...
        action: function () {
          !O.Fullscreen
            ? O.FullscreenTarget.requestFullscreen()
            : O.FullscreenTarget.ownerDocument.exitFullscreen();
          Config.close();
        },
      });
      O.FullscreenTarget.ownerDocument.addEventListener(
        "fullscreenchange",
        function () {
          // care multi-embeddeding
          if (!O.FullscreenButton)
            O.FullscreenButton = document.getElementById(
    ...
    if (Buttons.length) {
      const Section = (Config.WindowSection = Config.addSection({
        Labels: {
...
I.Menu.Config.LinkageSection = {
  create: (Components) => {
    const Config = I.Menu.Config;
    const Buttons = [];
    if (Components.includes("WebsiteLink"))
      Buttons.push({
        Type: "link",
        Labels: {
          default: {
            default: S["website-name-in-menu"]
              .replace(/&/gi, "&amp;")
              .replace(/</gi, "&lt;")
              .replace(/>/gi, "&gt;"),
          },
    ...
    if (Components.includes("BibiWebsiteLink"))
      Buttons.push({
        Type: "link",
    ...
    if (Buttons.length) {
      const Section = (Config.LinkageSection = Config.addSection({
        Labels: {
          default: {
            default: `Link${Buttons.length > 1 ? "s" : ""}`,
            ja: `リンク`,
          },
        },
      }));
...
I.Panel = {
  create: () => {
    const Panel = (I.Panel = O.Body.appendChild(
    ...
    Panel.BookInfo = Panel.appendChild(
      sML.create("div", { id: "bibi-panel-bookinfo" })
    );
    Panel.BookInfo.Cover = Panel.BookInfo.appendChild(
      sML.create("div", { id: "bibi-panel-bookinfo-cover" })
    );
    Panel.BookInfo.Cover.Info = Panel.BookInfo.Cover.appendChild(
      sML.create("p", { id: "bibi-panel-bookinfo-cover-info" })
    );
    const Opener = (Panel.Opener = I.Menu.L.addButtonGroup({
      Sticky: true,
    }).addButton({
      Type: "toggle",
      Labels: {
      ...
      Help: true,
      Icon: `<span class="bibi-icon bibi-icon-toggle-panel">${((Bars) => {
        for (let i = 1; i <= 6; i++) Bars += "<span></span>";
        return Bars;
      })("")}</span>`,
      action: () => Panel.toggle(),
    }));
    ...
    if (S["on-doubletap"] == "panel")
      E.add("bibi:doubletapped", () => Panel.toggle());
    if (S["on-tripletap"] == "panel")
      E.add("bibi:tripletapped", () => Panel.toggle());
...
I.Help = {
  create: () => {
    const Help = (I.Help = O.Body.appendChild(
      sML.create("div", { id: "bibi-help" })
    ));
    Help.Message = Help.appendChild(
    ...
    Help.hide = () => {
      Help.Timer_deactivate1 = setTimeout(() => {
        ...
        Help.Timer_deactivate2 = setTimeout(
...
I.PoweredBy = {
  create: () => {
    const PoweredBy = (I.PoweredBy = O.Body.appendChild(
      sML.create("div", {
        id: "bibi-poweredby",
        innerHTML: `<p><a href="${Bibi["href"]}" target="_blank" title="Bibi | Official Website">Bibi</a></p>`,
      })
    ));
    /*
    sML.appendCSSRule([ // Optimize to Scrollbar Size
...
I.FontSizeChanger = {
  create: () => {
    ...
    if (
      typeof S["font-size-scale-per-step"] != "number" ||
      S["font-size-scale-per-step"] <= 1
    )
      S["font-size-scale-per-step"] = 1.25;
    if (S["use-font-size-changer"] && S["keep-settings"] && O.Biscuits) {
      const BibiBiscuits = O.Biscuits.remember("Bibi");
      if (
        BibiBiscuits &&
        BibiBiscuits.FontSize &&
        BibiBiscuits.FontSize.Step != undefined
      )
        FontSizeChanger.Step = BibiBiscuits.FontSize.Step * 1;
    }
    if (
      typeof FontSizeChanger.Step != "number" ||
      FontSizeChanger.Step < -2 ||
      2 < FontSizeChanger.Step
    )
      FontSizeChanger.Step = 0;
    E.bind("bibi:postprocessed-item", (Item) => {
      if (Item["rendition:layout"] == "pre-paginated") return false;
      Item.changeFontSize = (FontSize) => {
        if (Item.FontSizeStyleRule)
          sML.deleteCSSRule(Item.contentDocument, Item.FontSizeStyleRule);
        Item.FontSizeStyleRule = sML.appendCSSRule(
          Item.contentDocument,
          "html",
          "font-size: " + FontSize + "px !important;"
        );
      };
      Item.changeFontSizeStep = (Step) =>
        Item.changeFontSize(
          Item.FontSize.Base * Math.pow(S["font-size-scale-per-step"], Step)
        );
      Item.FontSize = {
        Default:
          getComputedStyle(Item.HTML).fontSize.replace(/[^\d]*$/, "") * 1,
      };
      Item.FontSize.Base = Item.FontSize.Default;
      if (Item.Source.Preprocessed && (sML.UA.Chrome || sML.UA.Trident)) {
        sML.forEach(Item.HTML.querySelectorAll("body, body *"))(
          (Ele) =>
            (Ele.style.fontSize =
              parseInt(getComputedStyle(Ele).fontSize) / Item.FontSize.Base +
              "rem")
        );
      } else {
        O.editCSSRules(Item.contentDocument, (CSSRule) => {
          ...
          try {
            if (
              Item.contentDocument.querySelector(CSSRule.selectorText) ==
              Item.HTML
            )
              return;
          } catch (_) {}
          ...
          if (REs["pt"].test(CSSRule.cssText))
            CSSRule.style.fontSize =
              (CSSRule.cssText.match(REs["pt"])[1] * (96 / 72)) /
                Item.FontSize.Base +
              "rem";
          if (REs["px"].test(CSSRule.cssText))
            CSSRule.style.fontSize =
              CSSRule.cssText.match(REs["px"])[1] / Item.FontSize.Base + "rem";
        });
      }
      if (typeof S["base-font-size"] == "number" && S["base-font-size"] > 0) {
        ...
        sML.forEach(Item.Body.querySelectorAll("p, p *"))((Ele) => {
          ...
          const FontSize =
            Math.round(
              getComputedStyle(Ele).fontSize.replace(/[^\d]*$/, "") * 100
            ) / 100;
        ...
        if (MostPopularFontSize)
          Item.FontSize.Base =
            Item.FontSize.Base * (S["base-font-size"] / MostPopularFontSize);
        ...
        R.layOutBook({
          before: () =>
            R.Items.forEach((Item) => {
              if (Item.changeFontSizeStep) Item.changeFontSizeStep(Step);
            }),
          Reset: true,
    ...
    if (S["use-font-size-changer"]) {
      ...
      I.createSubpanel({
        ...
        .addSection({
          Labels: {
            default: { default: `Choose Font Size`, ja: `文字サイズを選択` },
          },
        })
        .addButtonGroup({
          //Tiled: true,
        ...
        .Buttons.forEach((Button) => {
          if (Button.Step == FontSizeChanger.Step)
            I.setUIState(Button, "active");
        });
    }
...
I.Loupe = {
  create: () => {
    ...
    if (S["loupe-scale-per-step"] > S["loupe-max-scale"])
      S["loupe-scale-per-step"] = S["loupe-max-scale"];
    const Loupe = (I.Loupe = {
      ...
      defineZoomOutPropertiesForUtilities: () => {
        ...
        if (S.ARA == "horizontal") {
          Tfm.Scale =
            (R.Main.offsetHeight - (BookMargin.Top + BookMargin.Bottom)) /
            (R.Main.offsetHeight -
              (S.ARA == S.SLA && (S.RVM != "paged" || I.Slider.ownerDocument)
                ? O.Scrollbars.Height
                : 0));
          Tfm.TranslateX = 0;
        } else {
          Tfm.Scale = Math.min(
            (R.Main.offsetWidth - BookMargin.Right) /
              (R.Main.offsetWidth - O.Scrollbars.Width),
        ...
        Tfm.TranslateY =
          BookMargin.Top - (R.Main.offsetHeight * (1 - Tfm.Scale)) / 2;
        const St =
          O.Body["offset" + C.A_SIZE_L] / Tfm.Scale -
          R.Main["offset" + C.A_SIZE_L];
        const OP = {};
        OP[C.A_BASE_B] =
          St / 2 +
          (!S["use-full-height"] && S.ARA == "vertical" ? I.Menu.Height : 0);
        OP[C.A_BASE_A] = St / 2;
        const IP = {};
        if (S.ARA == S.SLA)
          IP[S.ARA == "horizontal" ? "Right" : "Bottom"] = St / 2;
      ...
      getNormalizedTransformation: (Tfm) => {
        ...
        if (Tfm) {
          ...
          if (typeof Tfm.TranslateX == "number")
            NTfm.TranslateX = Tfm.TranslateX;
          if (typeof Tfm.TranslateY == "number")
            NTfm.TranslateY = Tfm.TranslateY;
        }
        return NTfm;
      },
      ...
      transform: (Tfm, Opt = {}) =>
        new Promise((resolve, reject) => {
          ...
          if (Tfm.Scale > 1) {
            ...
            Tfm.TranslateX = sML.limitMinMax(
              Tfm.TranslateX,
              OverflowX * -1 -
                (S.RVM != "vertical"
                  ? 0
                  : I.Slider.UIState == "active"
                  ? I.Slider.Size - O.Scrollbars.Width
                  : 0) +
                O.Scrollbars.Width,
              OverflowX
            );
            Tfm.TranslateY = sML.limitMinMax(
              Tfm.TranslateY,
              OverflowY * -1 -
                (S.RVM == "vertical"
                  ? 0
                  : I.Slider.UIState == "active"
                  ? I.Slider.Size - O.Scrollbars.Height
                  : 0) +
                O.Scrollbars.Height,
              OverflowY + (I.Menu.UIState == "active" ? I.Menu.Height : 0)
            );
          }
          ...
          sML.style(R.Main, {
            transform: ((Ps) => {
              if (ATfm.TranslateX && ATfm.TranslateY)
                Ps.push(
                  "translate(" +
                    ATfm.TranslateX +
                    "px" +
              ...
              else if (ATfm.TranslateX)
                Ps.push("translateX(" + ATfm.TranslateX + "px" + ")");
              else if (ATfm.TranslateY)
                Ps.push("translateY(" + ATfm.TranslateY + "px" + ")");
              if (ATfm.Scale != 1) Ps.push("scale(" + ATfm.Scale + ")");
              return Ps.length ? Ps.join(" ") : "";
            })([]),
          });
          Loupe.Timer_onTransformEnd = setTimeout(() => {
            if (Loupe.CurrentTransformation.Scale == 1)
              O.HTML.classList.remove("zoomed-in"),
                O.HTML.classList.remove("zoomed-out");
            else if (Loupe.CurrentTransformation.Scale < 1)
              O.HTML.classList.remove("zoomed-in"),
                O.HTML.classList.add("zoomed-out");
            else
              O.HTML.classList.add("zoomed-in"),
                O.HTML.classList.remove("zoomed-out");
      ...
      scale: (Scl, Opt = {}) => {
        ...
        Scl =
          typeof Scl == "number"
            ? sML.limitMinMax(Scl, 1, S["loupe-max-scale"])
            : 1;
        ...
        if (Scl < 1) {
          ...
          TX =
            CTfm.TranslateX +
            (Opt.Center.X - window.innerWidth / 2 - CTfm.TranslateX) *
              (1 - Scl / CTfm.Scale);
          TY =
            CTfm.TranslateY +
            (Opt.Center.Y - window.innerHeight / 2 - CTfm.TranslateY) *
              (1 - Scl / CTfm.Scale);
          /* ↑↑↑↑ SIMPLIFIED ↑↑↑↑
      ...
      transformToDefault: () =>
        Loupe.transform({ Scale: 1, TranslateX: 0, TranslateY: 0 }),
      transformForUtilities: (IO) => {
        ...
        if (IO) {
          ...
          const OP4U = Loupe.ZoomOutPropertiesForUtilities.OuterPadding,
            IP4U = Loupe.ZoomOutPropertiesForUtilities.InnerPadding;
          for (const Dir in OP4U)
            R.Main.style["padding" + Dir] = OP4U[Dir] + "px";
          for (const Dir in IP4U)
            R.Main.Book.style["padding" + Dir] = IP4U[Dir] + "px";
          Loupe.BookStretchingEach =
            Loupe.ZoomOutPropertiesForUtilities.Stretch / 2;
        } else {
          if (!Loupe.IsZoomedOutForUtilities) return Promise.resolve();
        ...
        return Loupe.transform(null, { Temporary: true })
          .then(cb)
          .then(() =>
            I.Slider.ownerDocument ? I.Slider.progress() : undefined
          );
      },
      isAvailable: () => {
        if (!L.Opened) return false;
      ...
      checkAndGetBibiEventForTaps: (Eve) => {
        ...
        if (BibiEvent.Target.tagName) {
          ...
          if (
            S.RVM == "horizontal" &&
            BibiEvent.Coord.Y > window.innerHeight - O.Scrollbars.Height
          )
            return null;
        }
        return BibiEvent;
      ...
      onTap: (Eve, HowManyTaps) => {
        if (
          HowManyTaps == 1 &&
          (!I.KeyObserver.ActiveKeys || !I.KeyObserver.ActiveKeys["Space"])
        )
          return Promise.resolve(); // Requires pressing space-key on single-tap.
        ...
        Eve.preventDefault();
        try {
          Eve.target.ownerDocument.body.Item.contentWindow
            .getSelection()
            .empty();
        } catch (Err) {}
        if (
          Loupe.CurrentTransformation.Scale >= S["loupe-max-scale"] &&
          !Eve.shiftKey
        )
          return Loupe.scale(1);
        return Loupe.scale(
          Loupe.CurrentTransformation.Scale *
            (Eve.shiftKey
              ? 1 / S["loupe-scale-per-step"]
              : S["loupe-scale-per-step"]),
          { Center: BibiEvent.Coord }
        );
      },
      onPointerDown: (Eve) => {
      ...
      onPointerMove: (Eve) => {
        ...
        if (Loupe.CurrentTransformation.Scale == 1 || !Loupe.PointerDownCoord)
          return false;
        Eve.preventDefault();
        Loupe.Dragging = true;
        ...
        Loupe.transform({
          Scale: Loupe.CurrentTransformation.Scale,
          TranslateX:
            Loupe.PointerDownTransformation.TranslateX +
            (BibiEvent.Coord.X - Loupe.PointerDownCoord.X),
          TranslateY:
            Loupe.PointerDownTransformation.TranslateY +
            (BibiEvent.Coord.Y - Loupe.PointerDownCoord.Y),
        });
        Loupe.Timer_TransitionRestore = setTimeout(
    ...
    I.isPointerStealth.addChecker(() => {
      ...
      if (!I.KeyObserver.ActiveKeys || !I.KeyObserver.ActiveKeys["Space"])
        return false;
      return true;
    });
    ...
    if (S["on-doubletap"] == "zoom")
      E.add("bibi:doubletapped", (Eve) => Loupe.onTap(Eve, 2));
    if (S["on-tripletap"] == "zoom")
      E.add("bibi:tripletapped", (Eve) => Loupe.onTap(Eve, 3));
    ...
    if (S["use-loupe"]) {
        ...
        Buttons: [
          {
            ...
            action: () =>
              Loupe.scale(
                Loupe.CurrentTransformation.Scale * S["loupe-scale-per-step"]
              ),
          ...
          },
          {
            ...
            updateState: function (State) {
              I.setUIState(
                ...
                typeof State == "string"
                  ? State
                  : Loupe.CurrentTransformation.Scale == 1
                  ? "disabled"
                  : "default"
              );
            },
      ...
      Loupe.updateButtonState = (State) =>
        ButtonGroup.Buttons.forEach((Button) => Button.updateState(State));
...
I.Nombre = {
  create: () => {
    ...
    const Nombre = (I.Nombre = O.Body.appendChild(
      sML.create("div", {
        id: "bibi-nombre",
        clearTimers: () => {
        ...
        hide: () => {
          ...
          Nombre.Timer_vanish = setTimeout(
        ...
        progress: (PageInfo) => {
          ...
          Nombre.Current.innerHTML = (() => {
            ...
            if (StartPageNumber != EndPageNumber)
              PageNumber += `<span class="delimiter">-</span>` + EndPageNumber;
            return PageNumber;
          })();
          ...
          Nombre.show();
          if (I.Slider.UIState != "active")
            Nombre.Timer_autohide = setTimeout(Nombre.hide, 1234);
        },
      })
    ));
    Nombre.Current = Nombre.appendChild(
      sML.create("span", { className: "bibi-nombre-current" })
    );
    Nombre.Delimiter = Nombre.appendChild(
      sML.create("span", { className: "bibi-nombre-delimiter" })
    );
    Nombre.Total = Nombre.appendChild(
      sML.create("span", { className: "bibi-nombre-total" })
    );
    Nombre.Percent = Nombre.appendChild(
    ...
    sML.appendCSSRule(
      ...
      "bottom: " + (O.Scrollbars.Height + 2) + "px;"
    );
    sML.appendCSSRule(
    ...
    sML.appendCSSRule(
      ...
      "right: " + (O.Scrollbars.Height + 2) + "px;"
    );
...
I.History = {
  ...
  add: (Opt = {}) => {
    ...
    const LastPage = R.hatchPage(I.History.List.slice(-1)[0]),
      CurrentPage = Opt.Destination
        ? R.hatchPage(Opt.Destination)
        : (() => {
            I.PageObserver.updateCurrent();
            return I.PageObserver.Current.List[0].Page;
          })();
    if (CurrentPage != LastPage) {
      if (Opt.SumUp && I.History.List.slice(-1)[0].UI == Opt.UI)
        I.History.List.pop();
      I.History.List.push({
        UI: Opt.UI,
        IIPP: I.PageObserver.getIIPP({ Page: CurrentPage }),
      });
      if (I.History.List.length - 1 > S["max-history"]) {
        // Not count the first (oldest).
...
I.Slider = {
  create: () => {
    ...
    const Slider = (I.Slider = O.Body.appendChild(
      sML.create("div", {
        id: "bibi-slider",
        RailProgressMode: "end", // or 'center'
        Size: I.Slider.Size,
        initialize: () => {
          const EdgebarBox = Slider.appendChild(
            sML.create("div", { id: "bibi-slider-edgebar-box" })
          );
          Slider.Edgebar = EdgebarBox.appendChild(
            sML.create("div", { id: "bibi-slider-edgebar" })
          );
          Slider.Rail = EdgebarBox.appendChild(
            sML.create("div", { id: "bibi-slider-rail" })
          );
          Slider.RailGroove = Slider.Rail.appendChild(
            sML.create("div", { id: "bibi-slider-rail-groove" })
          );
          Slider.RailProgress = Slider.RailGroove.appendChild(
            sML.create("div", { id: "bibi-slider-rail-progress" })
          );
          Slider.Thumb = EdgebarBox.appendChild(
            sML.create("div", {
          ...
          if (S["use-history"]) {
            ...
            Slider.History = Slider.appendChild(
              sML.create("div", { id: "bibi-slider-history" })
            );
            Slider.History.add = (Destination) =>
              I.History.add({
                UI: Slider,
                SumUp: false,
                Destination: Destination,
              });
            Slider.History.Button = Slider.History.appendChild(
              I.createButtonGroup()
            ).addButton({
              id: "bibi-slider-history-button",
              Type: "normal",
              Labels: {
                default: { default: `History Back`, ja: `移動履歴を戻る` },
              },
              Help: false,
              ...
              update: function () {
                this.Icon.style.transform = `rotate(${
                  360 * (I.History.List.length - 1)
                }deg)`;
                if (I.History.List.length <= 1) I.setUIState(this, "disabled");
                else if (this.UIState == "disabled")
                  I.setUIState(this, "default");
              },
            });
            I.History.Updaters.push(() => Slider.History.Button.update());
          }
          if (S["use-nombre"]) {
            E.add(Slider.Edgebar, ["mouseover", "mousemove"], (Eve) => {
              if (!Slider.Touching)
                I.Nombre.progress({
                  List: [
                    {
                      Page: Slider.getPointedPage(
                        O.getBibiEventCoord(Eve)[C.A_AXIS_L]
                      ),
                    },
                  ],
                });
            });
            E.add(Slider.Edgebar, "mouseout", (Eve) => {
              if (!Slider.Touching) I.Nombre.progress();
            });
          }
        },
        resetUISize: () => {
          Slider.MainLength = R.Main["scroll" + C.L_SIZE_L];
          const ThumbLengthPercent =
            (R.Main["offset" + C.L_SIZE_L] / Slider.MainLength) * 100;
          Slider.RailGroove.style[C.A_SIZE_b] = Slider.Thumb.style[C.A_SIZE_b] =
            "";
          Slider.RailGroove.style[C.A_SIZE_l] =
            100 -
            (Slider.RailProgressMode == "center" ? ThumbLengthPercent : 0) +
            "%";
          Slider.Thumb.style[C.A_SIZE_l] = ThumbLengthPercent + "%";
          setTimeout(
            () =>
              Slider.Thumb.classList.toggle(
                "min",
                ((STACS) => STACS.width == STACS.height)(
                  getComputedStyle(Slider.Thumb, "::after")
                )
              ),
            0
          );
          ...
          Slider.RailGroove.Before = O.getElementCoord(Slider.RailGroove)[
            C.A_AXIS_L
          ];
          Slider.RailGroove.Length = Slider.RailGroove["offset" + C.A_SIZE_L];
          Slider.RailGroove.After =
            Slider.RailGroove.Before + Slider.RailGroove.Length;
        ...
        onTouchStart: (Eve) => {
          ...
          Slider.StartedAt.Coord =
            Eve.target == Slider.Thumb
              ? O.getBibiEventCoord(Eve)[C.A_AXIS_L]
              : Slider.StartedAt.ThumbBefore + Slider.Thumb.Length / 2; // ← ? <Move Thumb naturally> : <Bring Thumb's center to the touched coord at the next pointer moving>
        ...
        onTouchMove: (Eve) => {
          ...
          if (Slider.Touching) {
            const Translation = sML.limitMinMax(
              TouchMoveCoord - Slider.StartedAt.Coord,
              Slider.Edgebar.Before - Slider.StartedAt.ThumbBefore,
              Slider.Edgebar.After -
                (Slider.StartedAt.ThumbBefore + Slider.Thumb.Length)
            );
            sML.style(Slider.Thumb, {
              transform: "translate" + C.A_AXIS_L + "(" + Translation + "px)",
            });
            sML.style(Slider.RailProgress, {
              [C.A_SIZE_l]:
                Slider.StartedAt.RailProgressLength +
                Translation * (S.ARD == "rtl" ? -1 : 1) +
                "px",
            });
          }
          Slider.flipPagesDuringSliding(TouchMoveCoord);
        },
        flipPagesDuringSliding: (_) =>
          (Slider.flipPagesDuringSliding = !S["flip-pages-during-sliding"] // switch and define at the first call.
            ? () => false
            : (TouchMoveCoord) => {
                R.DoNotTurn = true;
                Slider.flip(TouchMoveCoord);
                Slider.Timer_move = setTimeout(() => {
                  R.DoNotTurn = false;
                  Slider.flip(TouchMoveCoord, "FORCE");
                }, 333);
              })(_),
        onTouchEnd: (Eve) => {
          ...
          if (TouchEndCoord == Slider.StartedAt.Coord)
            Slider.StartedAt.Coord =
              Slider.StartedAt.ThumbBefore + Slider.Thumb.Length / 2;
          R.DoNotTurn = false;
          ...
          Slider.Timer_onTouchEnd = setTimeout(
            () => O.HTML.classList.remove("slider-sliding"),
            123
          );
        },
        flip: (TouchedCoord, Force) =>
          new Promise((resolve) => {
            switch (S.RVM) {
              case "paged":
                const TargetPage = Slider.getPointedPage(TouchedCoord);
                return I.PageObserver.Current.Pages.includes(TargetPage)
                  ? resolve()
                  : R.focusOn({
              ...
              default:
                R.Main["scroll" + C.L_OOBL_L] =
                  Slider.StartedAt.MainScrollBefore +
                  (TouchedCoord - Slider.StartedAt.Coord) *
                    (Slider.MainLength / Slider.Edgebar.Length);
                return resolve();
            }
          }).then(() => {
        ...
        progress: () => {
          ...
          if (S.ARA != S.SLA && S.ARD == "rtl")
            MainScrollBefore =
              Slider.MainLength - MainScrollBefore - R.Main.offsetHeight; // <- Paged (HorizontalAppearance) && VerticalText
          switch (S.ARA) {
            case "horizontal":
              (Slider.Thumb.style.top = ""),
                (Slider.RailProgress.style.height = "");
              break;
            case "vertical":
              (Slider.Thumb.style.left = ""),
                (Slider.RailProgress.style.width = "");
              break;
          }
          Slider.Thumb.style[C.A_OOBL_l] =
            (MainScrollBefore / Slider.MainLength) * 100 + "%";
          Slider.RailProgress.style[C.A_SIZE_l] =
            (Slider.getRailProgressLength(
              O.getElementCoord(Slider.Thumb)[C.A_AXIS_L] -
                Slider.RailGroove.Before
            ) /
              Slider.RailGroove.Length) *
              100 +
            "%";
        },
        getRailProgressLength: (_) =>
          (Slider.getRailProgressLength =
            Slider.RailProgressMode == "center" // switch and define at the first call.
              ? (ThumbBeforeInRailGroove) =>
                  S.ARD != "rtl"
                    ? ThumbBeforeInRailGroove + Slider.Thumb.Length / 2
                    : Slider.RailGroove.Length -
                      (ThumbBeforeInRailGroove + Slider.Thumb.Length / 2)
              : (ThumbBeforeInRailGroove) =>
                  S.ARD != "rtl"
                    ? ThumbBeforeInRailGroove + Slider.Thumb.Length
                    : Slider.RailGroove.Length - ThumbBeforeInRailGroove)(_),
        getPointedPage: (PointedCoord) => {
          let RatioInSlider =
            (PointedCoord - Slider.Edgebar.Before) /
            Slider.Edgebar["offset" + C.A_SIZE_L];
          const OriginPageIndex = sML.limitMinMax(
            Math.round(
              R.Pages.length *
                (S.ARD == "rtl" ? 1 - RatioInSlider : RatioInSlider)
            ),
            0,
            R.Pages.length - 1
          );
          const PointedCoordInBook =
            R.Main["scroll" + C.L_SIZE_L] *
            (S.ARD == "rtl" && S.SLD == "ttb"
              ? 1 - RatioInSlider
              : RatioInSlider);
          let ThePage = R.Pages[OriginPageIndex],
            MinDist = Slider.getPageDistanceFromPoint(
              ThePage,
              PointedCoordInBook
            );
          [-1, 1].forEach((PM) => {
            for (let i = OriginPageIndex + PM; R.Pages[i]; i += PM) {
              const Page = R.Pages[i],
                Dist = Slider.getPageDistanceFromPoint(
                  Page,
                  PointedCoordInBook
                );
              if (Dist < MinDist) (ThePage = Page), (MinDist = Dist);
              else break;
            }
          });
          return ThePage;
        ...
        getPageDistanceFromPoint: (Page, PointedCoordInBook) => {
          return Math.abs(
            PointedCoordInBook -
              (O.getElementCoord(Page, R.Main)[C.L_AXIS_L] +
                Page["offset" + C.L_SIZE_L] * 0.5)
          );
        },
      })
    ));
    ...
    I.setToggleAction(Slider, {
      ...
      onclosed: () => {
        new Promise((resolve) =>
          setTimeout(resolve, S["zoom-out-for-utilities"] ? 111 : 0)
        );
    ...
    E.add("bibi:opens-utilities", (Opt) =>
      E.dispatch("bibi:commands:open-slider", Opt)
    );
    E.add("bibi:closes-utilities", (Opt) =>
      E.dispatch("bibi:commands:close-slider", Opt)
    );
    E.add("bibi:loaded-item", (Item) =>
      Item.HTML.addEventListener(E["pointerup"], Slider.onTouchEnd)
    );
    E.add("bibi:opened", () => {
      ...
      if (Slider.History)
        Slider.History.Button.addEventListener(
          E["pointerup"],
          Slider.onTouchEnd
        );
    ...
    {
      // Optimize to Scrollbar Size
      const _S = "div#bibi-slider",
        _TB = "-thumb:before";
      const _HS = "html.appearance-horizontal " + _S,
        _HSTB = _HS + _TB,
        _SH = O.Scrollbars.Height,
        _STH = Math.ceil(_SH / 2);
      const _VS = "html.appearance-vertical " + _S,
        _VSTB = _VS + _TB,
        _SW = O.Scrollbars.Width,
        _STW = Math.ceil(_SW / 2);
      const _getSliderThumbOffsetStyle = (Offset) =>
        ["top", "right", "bottom", "left"]
          .reduce((Style, Dir) => Style + Dir + ": " + Offset * -1 + "px; ", "")
          .trim();
      sML.appendCSSRule(_HS, "height: " + _SH + "px;");
      sML.appendCSSRule(
        _HSTB,
        _getSliderThumbOffsetStyle(_STH) +
          " border-radius: " +
          _STH / 2 +
          "px; min-width: " +
          _STH +
      ...
      sML.appendCSSRule(
        _VSTB,
        _getSliderThumbOffsetStyle(_STW) +
          " border-radius: " +
          _STW / 2 +
          "px; min-height: " +
          _STW +
...
I.BookmarkManager = {
  create: () => {
    if (!S["use-bookmarks"] || !O.Biscuits) return;
    const BookmarkManager = (I.BookmarkManager = {
      ...
      initialize: () => {
        BookmarkManager.Subpanel = I.createSubpanel({
          ...
          onopened: () => {
            E.add("bibi:scrolled", BookmarkManager.Subpanel.updateBookmarks);
            BookmarkManager.Subpanel.updateBookmarks();
          },
          onclosed: () => {
            E.remove("bibi:scrolled", BookmarkManager.Subpanel.updateBookmarks);
          },
        });
        ...
        if (Array.isArray(BookmarkBiscuits) && BookmarkBiscuits.length)
      ...
      exists: (Bookmark) => {
        for (let l = BookmarkManager.Bookmarks.length, i = 0; i < l; i++)
          if (BookmarkManager.Bookmarks[i].IIPP == Bookmark.IIPP)
            return BookmarkManager.Bookmarks[i];
        return null;
      },
      add: (Bookmark) => {
      ...
      remove: (Bookmark) => {
        BookmarkManager.Bookmarks = BookmarkManager.Bookmarks.filter(
          (Bmk) => Bmk.IIPP != Bookmark.IIPP
        );
      ...
      update: (Opt = {}) => {
        ...
        let Bookmarks = [],
          ExistingBookmarks = [];
        if (Array.isArray(Opt.Bookmarks))
          BookmarkManager.Bookmarks = Opt.Bookmarks;
        ...
        if (BookmarkManager.Bookmarks.length) {
          ...
          for (let l = BookmarkManager.Bookmarks.length, i = 0; i < l; i++) {
            ...
            if (Page && typeof Page.Index == "number")
              ...
              PageNumber = Math.floor(Bmk.IIPP) + 1;
            if (PageNumber) {
            ...
            if (typeof Bmk["%"] == "number") {
              if (Label)
                ...
                Label +=
                  `<span class="${BB}-percent">` +
                  `<span class="${BB}-number">${Bmk["%"]}</span><span class="${BB}-unit">%</span>` +
                  `</span>`;
            }
            const Labels = Label
              ? { default: { default: Label, ja: Label } }
              : {
                  default: {
                    default: `Bookmark #${UpdatedBookmarks.length + 1}`,
                    ja: `しおり #${UpdatedBookmarks.length + 1}`,
                  },
                };
            if (
              Bookmarks.reduce(
                (Exists, Bookmark) =>
                  (Exists = Bmk.IIPP == Bookmark.IIPP ? true : Exists),
                false
              )
            ) {
            ...
            const Button = BookmarkManager.ButtonGroup.addButton({
              ...
              action: () => {
                if (L.Opened)
                  return R.focusOn({ Destination: Bmk }).then((Destination) =>
                    I.History.add({
                      UI: BookmarkManager,
                      SumUp: false /*true*/,
                      Destination: Destination,
            ...
            const Remover = Button.appendChild(
              sML.create("span", {
            ...
            Remover.addEventListener(E["pointer-over"], (Eve) =>
              Eve.stopPropagation()
            );
            if (Bmk.IsHot) {
              delete Bmk.IsHot;
              ...
              setTimeout(
                () =>
                  I.setUIState(
                    Button,
                    ExistingBookmarks.includes(Bmk) ? "disabled" : "default"
                  ),
                234
              );
            } else if (ExistingBookmarks.includes(Bmk))
              I.setUIState(Button, "disabled");
        ...
        if (BookmarkManager.Bookmarks.length < S["max-bookmarks"]) {
          BookmarkManager.AddButton = BookmarkManager.ButtonGroup.addButton({
            ...
            action: () =>
              Bookmarks.length ? BookmarkManager.add(Bookmarks[0]) : false,
          });
        ...
        if (Opt.Added)
          E.dispatch("bibi:added-bookmark", BookmarkManager.Bookmarks);
        if (Opt.Removed)
          E.dispatch("bibi:removed-bookmark", BookmarkManager.Bookmarks);
      },
    });
  ...
  create: () => {
    const Flipper = (I.Flipper = {
      ...
      getDirection: (Division) => {
        switch (S.ARA) {
          case "horizontal":
            return Division.X != "center" ? Division.X : Division.Y;
          case "vertical":
            return Division.Y != "middle" ? Division.Y : Division.X;
        }
      },
      isAbleToFlip: (Distance) => {
        if (
          ...
          if (I.PageObserver.Current.List.length) {
            ...
            if (Distance < 0)
              (CurrentEdge = I.PageObserver.Current.List[0]),
                (BookEdgePage = R.Pages[0]),
                (Edged = "Headed");
            else
              (CurrentEdge = I.PageObserver.Current.List.slice(-1)[0]),
                (BookEdgePage = R.Pages.slice(-1)[0]),
                (Edged = "Footed");
            if (CurrentEdge.Page != BookEdgePage) return true;
            if (
              !CurrentEdge.PageIntersectionStatus.Contained &&
              !CurrentEdge.PageIntersectionStatus[Edged]
            )
              return true;
          }
        }
      ...
      flip: (Distance, Opt = {}) => {
        ...
        I.ScrollObserver.forceStopScrolling();
        const SumUpHistory =
          I.History.List.slice(-1)[0].UI == Flipper &&
          (Distance < 0 ? -1 : 1) === (Flipper.PreviousDistance < 0 ? -1 : 1);
        ...
        if (S["book-rendition-layout"] == "pre-paginated") {
          ...
          CIs.forEach((CI) => {
            try {
              R.Pages[CI].Spread.Box.classList.remove("current");
            } catch (Err) {}
          });
          try {
            R.Pages[TI].Spread.Box.classList.add("current");
          } catch (Err) {}
        }
        return R.moveBy({
          Distance: Distance,
...
I.Arrows = {
  create: () => {
    if (!S["use-arrows"]) return (I.Arrows = null);
    const Arrows = (I.Arrows = {
      ...
      areAvailable: (BibiEvent) => {
        ...
        else if (S.ARA == "horizontal") {
          if (
            BibiEvent.Coord.X >= window.innerWidth - Buf ||
            BibiEvent.Coord.Y >= window.innerHeight - O.Scrollbars.Height - Buf
          )
            return false;
        } else if (S.ARA == "vertical") {
          if (
            BibiEvent.Coord.Y >= window.innerHeight - Buf ||
            BibiEvent.Coord.X >= window.innerWidth - O.Scrollbars.Width - Buf
          )
            return false;
        }
        if (BibiEvent.Target.ownerDocument.documentElement == O.HTML) {
          if (
            BibiEvent.Target == O.HTML ||
            BibiEvent.Target == O.Body ||
            BibiEvent.Target == I.Menu
          )
            return true;
          if (
            ...
            return true;
          if (/^(spread|item|page)( |-|$)/.test(BibiEvent.Target.className))
            return true;
        } else {
    ...
    Arrows.Back = O.Body.appendChild(
      sML.create("div", {
    ...
    Arrows.Forward = O.Body.appendChild(
      sML.create("div", {
        className: "bibi-arrow",
    ...
    [Arrows.Back, Arrows.Forward].forEach((Arrow) => {
      ...
      const FunctionsToBeCanceled = [
        Arrow.showHelp,
        Arrow.hideHelp,
        Arrow.BibiTapObserver.onTap,
        Arrow.BibiTapObserver.onDoubleTap,
      ];
      if (!O.TouchOS)
        FunctionsToBeCanceled.push(
          Arrow.BibiHoverObserver.onHover,
          Arrow.BibiHoverObserver.onUnHover
        );
      FunctionsToBeCanceled.forEach((f2BC) => (f2BC = () => {}));
    });
    E.add("bibi:commands:move-by", (Distance) => {
      // indicate direction
      if (
        !L.Opened ||
        typeof (Distance *= 1) != "number" ||
        ...
        return false;
      return E.dispatch(
        Distance < 0 ? Arrows.Back : Arrows.Forward,
        "bibi:tapped",
        null
      );
    });
    ...
    ((_) => {
      ...
      _(
        ...
        "height",
        O.Scrollbars.Width + I.Menu.Height
      );
      _(
    ...
    })((Context, WidthOrHeight, Margin) =>
      sML.appendCSSRule(
        `${Context} div#bibi-arrow-back, ${Context} div#bibi-arrow-forward`,
        `${WidthOrHeight}: calc(100% - ${Margin}px); ${WidthOrHeight}: calc(100v${WidthOrHeight.charAt(
          0
        )} - ${Margin}px);`
      )
    );
...
I.AxisSwitcher = {
  create: () => {
    if (S["fix-reader-view-mode"]) return (I.AxisSwitcher = null);
    const AxisSwitcher = (I.AxisSwitcher = {
      switchAxis: () =>
        new Promise((resolve) => {
          if (S.RVM == "paged") return resolve();
          const ViewMode = S.RVM == "horizontal" ? "vertical" : "horizontal";
          I.Menu.Config.ViewModeSection.ButtonGroups[0].Buttons.filter(
            (Button) => Button.Mode == ViewMode
...
I.Spinner = {
  create: () => {
    const Spinner = (I.Spinner = O.Body.appendChild(
...
I.createButtonGroup = (Par = {}) => {
  ...
  const ButtonsToAdd = Array.isArray(Par.Buttons)
    ? Par.Buttons
    : Par.Button
    ? [Par.Button]
    : [];
  ...
  ButtonGroup.addButton = function (Par) {
    ...
    Button.ButtonBox = Button.ButtonGroup.appendChild(
      sML.create("li", {
        className: "bibi-buttonbox bibi-buttonbox-" + Button.Type,
      })
    );
    if (!O.TouchOS) {
...
I.createButton = (Par = {}) => {
  ...
  if (typeof Par.id != "string" || !Par.id) delete Par.id;
  Par.Type =
    typeof Par.Type == "string" && /^(normal|toggle|radio|link)$/.test(Par.Type)
      ? Par.Type
      : "normal";
  ...
  if (Button.Icon) {
    Button.IconBox = Button.appendChild(
  ...
  Button.Label = Button.appendChild(
  ...
  if (typeof Button.action == "function")
    E.add(Button, "bibi:tapped", () =>
      Button.isAvailable() ? Button.action.apply(Button, arguments) : null
    );
  Button.Busy = false;
  return Button;
};
...
I.createSubpanel = (Par = {}) => {
  ...
  const ClassNames = [
    "bibi-subpanel",
    "bibi-subpanel-" + (Par.Position == "left" ? "left" : "right"),
  ];
  if (Par.className) ClassNames.push(Par.className);
  Par.className = ClassNames.join(" ");
  const SectionsToAdd = Array.isArray(Par.Sections)
    ? Par.Sections
    : Par.Section
    ? [Par.Section]
    : [];
  ...
  I.setToggleAction(Subpanel, {
    onopened: function (Opt) {
      I.Subpanels.forEach((Sp) =>
        Sp == Subpanel ? true : Sp.close({ ForAnotherSubpanel: true })
      );
    ...
    onclosed: function (Opt) {
      ...
      if (I.OpenedSubpanel == this)
        setTimeout(() => (I.OpenedSubpanel = null), 222);
...
I.createSubpanelSection = (Par = {}) => {
  ...
  if (Par.className) ClassNames.push(Par.className);
  Par.className = ClassNames.join(" ");
  const PGroupsToAdd = Array.isArray(Par.PGroups)
    ? Par.PGroups
    : Par.PGroup
    ? [Par.PGroup]
    : [];
  delete Par.PGroups;
  delete Par.PGroup;
  const ButtonGroupsToAdd = Array.isArray(Par.ButtonGroups)
    ? Par.ButtonGroups
    : Par.ButtonGroup
    ? [Par.ButtonGroup]
    : [];
  ...
  if (SubpanelSection.Labels) {
    ...
    SubpanelSection.appendChild(sML.create("div", { className: "bibi-hgroup" }))
      .appendChild(sML.create("p", { className: "bibi-h" }))
      .appendChild(
        sML.create("span", {
          className: "bibi-h-label",
          innerHTML: SubpanelSection.Labels["default"][O.Language],
        })
      );
  }
...
I.setToggleAction = (Obj, Par = {}) => {
  ...
  return sML.edit(Obj, {
    ...
    toggle: (Opt) =>
      Obj.UIState == "default" ? Obj.open(Opt) : Obj.close(Opt),
  });
};
...
I.setFeedback = (Ele, Opt = {}) => {
  ...
  if (Ele.Labels) {
    if (Opt.Help) {
      Ele.showHelp = () => {
        if (I.Help && Ele.Labels[Ele.UIState])
          I.Help.show(Ele.Labels[Ele.UIState][O.Language]);
        return Ele;
      };
      Ele.hideHelp = () => {
    ...
    if (Ele.Notes)
      Ele.note = () => {
        if (Ele.Labels[Ele.UIState])
          setTimeout(() => I.note(Ele.Labels[Ele.UIState][O.Language]), 0);
        return Ele;
      };
  }
  if (!O.TouchOS) {
...
I.setUIState = (UI, UIState) => {
  ...
  if (UI.tagName) {
    if (
      UI.Labels &&
      UI.Labels[UI.UIState] &&
      UI.Labels[UI.UIState][O.Language]
    ) {
...
I.isPointerStealth = () => {
  ...
  I.isPointerStealth.Checkers.forEach(
    (checker) => (IsPointerStealth = checker() ? true : IsPointerStealth)
  );
...
I.isPointerStealth.addChecker = (fun) =>
  typeof fun == "function" && !I.isPointerStealth.Checkers.includes(fun)
    ? I.isPointerStealth.Checkers.push(fun)
    : I.isPointerStealth.Checkers.length;

I.distillLabels = (Labels) => {
  if (typeof Labels != "object" || !Labels) Labels = {};
  for (const State in Labels)
    Labels[State] = I.distillLabels.distillLanguage(Labels[State]);
  if (!Labels["default"]) Labels["default"] = I.distillLabels.distillLanguage();
  if (!Labels["active"] && Labels["default"])
    Labels["active"] = Labels["default"];
  if (!Labels["disabled"] && Labels["default"])
    Labels["disabled"] = Labels["default"];
  return Labels;
};
...
I.distillLabels.distillLanguage = (Label) => {
  ...
  if (typeof Label["default"] != "string") {
    ...
    else if (typeof Label[O.Language] == "string")
      Label["default"] = Label[O.Language];
    else Label["default"] = "";
  }
  if (typeof Label[O.Language] != "string") {
    if (typeof Label["default"] == "string")
      Label[O.Language] = Label["default"];
...
I.orthogonal = (InputType) =>
  S["orthogonal-" + InputType][S.RVM == "paged" ? 0 : 1];

I.isScrollable = () =>
  S.ARA == S.SLA && I.Loupe.CurrentTransformation.Scale == 1 ? true : false;

I.getBookIcon = () =>
  sML.create("div", { className: "book-icon", innerHTML: `<span></span>` });
...
Bibi.preset = (Preset) =>
  Bibi.at1st.List.push(() => {
    Bibi.applyFilteredSettingsTo(
      P,
      Preset,
      [Bibi.SettingTypes, Bibi.SettingTypes_PresetOnly],
      "Fill"
    );
    delete P["book"];
...
P.initialize = () => {
  ...
  P["bookshelf"] = new URL(
    P["bookshelf"] || "../../bibi-bookshelf",
    P.Script.src
  ).href.replace(/\/$/, "");
  P["extensions"] = (() => {
    let Extensions_HTML = document
      .getElementById("bibi-preset")
      .getAttribute("data-bibi-extensions");
    if (Extensions_HTML) {
      Extensions_HTML = Extensions_HTML.trim()
        .replace(/\s+/, " ")
        .split(" ")
        .map((EPath) => ({ src: new URL(EPath, DocHRef).href }));
      if (Extensions_HTML.length) P["extensions"] = Extensions_HTML;
    }
    return !Array.isArray(P["extensions"])
      ? []
      : P["extensions"].filter((Xtn) => {
          if (Xtn.hasOwnProperty("-spell-of-activation-")) {
            const SoA = Xtn["-spell-of-activation-"];
            if (
              !SoA ||
              !/^[a-zA-Z0-9_\-]+$/.test(SoA) ||
              !U.hasOwnProperty(SoA)
            )
              return false;
          }
          if (!Xtn || !Xtn["src"] || typeof Xtn["src"] != "string")
            return false;
...
Bibi.at1st.List.unshift(() => {
  ...
  Object.assign(
    U,
    Bibi.applyFilteredSettingsTo(Q, Q, [
      Bibi.SettingTypes,
      Bibi.SettingTypes_UserOnly,
    ])
  );
...
U.initialize = () => {
  const _U = Bibi.applyFilteredSettingsTo({}, U, [
    Bibi.SettingTypes,
    Bibi.SettingTypes_UserOnly,
  ]);
  const HashData = (() => {
    ...
    for (const Cat in Data) {
      ...
      Data[Cat] = Bibi.applyFilteredSettingsTo({}, ParsedData, [
        Bibi.SettingTypes,
        Bibi.SettingTypes_UserOnly,
      ]);
  ...
  if (HashData["#"]) {
    Object.assign(_U, (_U["#"] = HashData["#"]));
  }
  if (HashData["Bibi"]) {
    Object.assign(_U, (_U["Bibi"] = HashData["Bibi"]));
  }
  if (HashData["Jo"]) {
    Object.assign(_U, (_U["Jo"] = HashData["Jo"]));
  ...
  Object.assign(U, _U);
  if (typeof U["nav"] == "number")
    U["nav"] < 1 ? delete U["nav"] : (R.StartOn = { Nav: U["nav"] });
  // to be converted in L.coordinateLinkages
  ...
  else if (typeof U["EPUBCFI"] == "string")
    E.add("bibi:readied", () => {
      if (X["EPUBCFI"]) R.StartOn = X["EPUBCFI"].getDestination(U["EPUBCFI"]);
    });
};
...
D.initialize = () => {
  ...
  if (BookDataElement) {
    ...
    if (BookData) {
      const BookDataMIMEType = BookDataElement.getAttribute(
        "data-bibi-book-mimetype"
      );
      if (
        /^application\/(epub\+zip|zip|x-zip(-compressed)?)$/i.test(
          BookDataMIMEType
        )
      ) {
...
S.initialize = () => {
  ...
  Bibi.SettingTypes["yes-no"]
    .concat(Bibi.SettingTypes_PresetOnly["yes-no"])
    .concat(Bibi.SettingTypes_UserOnly["yes-no"])
    .forEach(
      (Pro) =>
        (S[Pro] =
          S[Pro] == "yes" ||
          (S[Pro] == "mobile" && O.TouchOS) ||
          (S[Pro] == "desktop" && !O.TouchOS))
    );
  // --------
  if (!S["trustworthy-origins"].includes(O.Origin))
    S["trustworthy-origins"].unshift(O.Origin);
  // --------
  S["book"] =
    !S["book-data"] && typeof S["book"] == "string" && S["book"]
      ? new URL(S["book"], S["bookshelf"] + "/").href
      : "";
  if (
    !S["book-data"] &&
    S["book"] &&
    !S["trustworthy-origins"].includes(new URL(S["book"]).origin)
  )
    throw `The Origin of the Path of the Book Is Not Allowed.`;
  // --------
  if (typeof S["parent-bibi-index"] != "number") delete S["parent-bibi-index"];
  // --------
  if (S["book"] || !window.File)
    (S["accept-local-file"] = false),
      (S["accept-blob-converted-data"] = false),
      (S["accept-base64-encoded-data"] = false);
  else
    S["accept-local-file"] =
      S["accept-local-file"] &&
      (S["extract-if-necessary"].includes("*") ||
        S["extract-if-necessary"].includes(".epub") ||
        S["extract-if-necessary"].includes(".zip"))
        ? true
        : false;
  // --------
  S["autostart"] = S["wait"]
    ...
    ? S["autostart-embedded"]
    : S["autostart"];
  S["start-in-new-window"] =
    window.parent != window && !S["autostart"]
      ? S["start-embedded-in-new-window"]
      : false;
  // --------
  S["default-page-progression-direction"] =
    S["default-page-progression-direction"] == "rtl" ? "rtl" : "ltr";
  ...
  if (O.Biscuits)
    E.bind("bibi:initialized-book", () => {
      ...
      if (S["keep-settings"]) {
        if (!U["reader-view-mode"] && BookBiscuits.RVM)
          S["reader-view-mode"] = BookBiscuits.RVM;
        if (!U["full-breadth-layout-in-scroll"] && BookBiscuits.FBL)
          S["full-breadth-layout-in-scroll"] = BookBiscuits.FBL;
      }
      if (S["resume-from-last-position"]) {
        if (!R.StartOn && BookBiscuits.Position && BookBiscuits.Position.IIPP)
          R.StartOn = sML.clone(BookBiscuits.Position);
      }
    });
  // --------
  S.Modes = {
    // 'Mode': { SH: 'ShortHand', CNP: 'ClassNamePrefix' }
...
S.update = (Settings) => {
  const Prev = {};
  for (const Mode in S.Modes) Prev[Mode] = S[Mode];
  if (typeof Settings == "object")
    for (const Property in Settings)
      if (typeof S[Property] != "function") S[Property] = Settings[Property];
  ...
  if (S["ui-font-family"])
    S.FontFamilyStyleIndex = sML.appendCSSRule(
      "html",
      "font-family: " + S["ui-font-family"] + " !important;"
    );
  ...
  S["apparent-reading-axis"] =
    S["reader-view-mode"] == "paged" ? "horizontal" : S["reader-view-mode"];
  S["apparent-reading-direction"] =
    S["reader-view-mode"] == "vertical"
      ? "ttb"
      : S["page-progression-direction"];
  S["spread-layout-direction"] =
    S["spread-layout-axis"] == "vertical"
      ? "ttb"
      : S["page-progression-direction"];
  S["navigation-layout-direction"] =
    S["fix-nav-ttb"] || S["page-progression-direction"] != "rtl"
      ? "ttb"
      : "rtl";
  //S['spread-gap'] = S['book-rendition-layout'] == 'reflowable' ? S['spread-gap_reflowable'] : S['spread-gap_pre-paginated'];
  for (const Mode in S.Modes) {
    const Pfx = S.Modes[Mode].CNP + "-",
      PC = Pfx + Prev[Mode],
      CC = Pfx + S[Mode];
...
C.update = () => {
  ...
  C.DDD = (() => {
    switch (
      S.PPD // DDD: Direction-Distance Dictionary
    ) {
      case "ltr":
        return S.ARD != "ttb"
...
C.probe = (L_A, AXIS) => {
  const LR_RL = ["left", "right"];
  if (S.PPD != "ltr") LR_RL.reverse();
  if (AXIS == "horizontal") {
    ...
    C._app(L_A, "AXIS", { b: "y", l: "x" });
    C[L_A + "_AXIS_D"] = S.PPD == "ltr" ? 1 : -1;
  } else {
...
C._app = (L_A, Gauge, Par) => {
  for (const Pro in Par)
    (C[[L_A, Gauge, Pro].join("_")] = Par[Pro]),
      (C[[L_A, Gauge, sML.capitalise(Pro)].join("_")] = sML.capitalise(
        Par[Pro]
      ));
};

C.d2d = (Dir, AOD) => {
  const Dist = C.DDD[Dir];
  return AOD ? Dist * 1 : typeof Dist == "number" ? Dist : 0;
}; // d2d: Direction to Distance // AOD: Allow Orthogonal Direction
  ...
  if (
    ...
    if (Log)
      switch (Tag) {
        case "<b:>":
          Ls.unshift(`📕`);
          Ls.push("%c" + Log), Ss.push(O.log.BStyle);
          Ls.push(`%c(v${Bibi["version"]})` + (Bibi.Dev ? ":%cDEV" : "")),
            Ss.push(O.log.NStyle);
          if (Bibi.Dev) Ss.push(O.log.BStyle);
          break;
        case "</b>":
          Ls.unshift(`📖`);
          Ls.push("%c" + Log), Ss.push(O.log.BStyle);
...
O.log.initialize = () => {
  ...
  O.log.Limit =
    U.hasOwnProperty("log") && typeof (U["log"] *= 1) == "number"
      ? U["log"]
      : 0;
  O.log.Depth = 1;
...
O.getTimeLabel = (TimeFromOrigin = Date.now() - Bibi.TimeOrigin) =>
  [
    ...
    .join(":") +
  "." +
  String(TimeFromOrigin % 1000).padStart(3, 0);
...
O.src = (Source) => {
  if (!B.Package.Manifest[Source.Path])
    B.Package.Manifest[Source.Path] = Source;
  if (!Source["media-type"])
    Source["media-type"] = O.getContentType(Source.Path);
...
O.cancelExtraction = (Source) => {
  if (Source.Resources)
    Source.Resources.forEach((Res) =>
      Res.Retlieved ? Promise.resolve() : O.RangeLoader.abort(Res.Path)
    );
  return Source.Retlieved
    ? Promise.resolve()
    : O.RangeLoader.abort(Source.Path);
};
...
O.extract = (Source) => {
  ...
  return (Source.Retlieving = O.RangeLoader.getBuffer(Source.Path)
    .then((ABuf) => {
      if (O.isBin(Source))
        (Source.DataType = "Blob"),
          (Source.Content = new Blob([ABuf], { type: Source["media-type"] }));
      else
        (Source.DataType = "Text"),
          (Source.Content = new TextDecoder("utf-8").decode(
            new Uint8Array(ABuf)
          ));
    ...
    .catch((Err) => {
      ...
      return Promise.reject(
        /404/.test(Err)
          ? Bibi.ErrorMessages.NotFound
          : /aborted/.test(Err)
          ? Bibi.ErrorMessages.Canceled
          : /fetch/.test(Err)
          ? Bibi.ErrorMessages.CORSBlocked
          : /not found/.test(Err)
          ? Bibi.ErrorMessages.DataInvalid
          : /invalid/.test(Err)
          ? Bibi.ErrorMessages.DataInvalid
          : Err
      );
...
O.download = (Source /*, Opt = {}*/) => {
  ...
  const RemotePath = Source.URI
    ? Source.URI
    : (/^([a-z]+:\/\/|\/)/.test(Source.Path) ? "" : B.Path + "/") + Source.Path;
...
O.tryRangeRequest = (Path = Bibi.Script.src, Bytes = "0-0") =>
  new Promise((resolve, reject) => {
  ...
  new Promise((resolve, reject) => {
    ...
    (() => {
      ...
      switch (B.ExtractionPolicy) {
        case "on-the-fly":
          return O.extract(Source);
        case "at-once":
          return Promise.reject(`File Not Included: "${Source.Path}"`);
      }
    })()
      .then((Source) => {
        if (typeof Opt.initialize == "function") Opt.initialize(Source);
        return Opt.Preprocess && !Source.Preprocessed
          ? O.preprocess(Source)
          : Source;
      })
      .then((Source) => {
...
O.isBin = (Source) =>
  /\.(aac|gif|jpe?g|m4[av]|mp[g34]|ogg|[ot]tf|pdf|png|web[mp]|woff2?)$/i.test(
    Source.Path
  );

O.createBlobURL = (DT, CB, MT) =>
  URL.createObjectURL(DT == "Text" ? new Blob([CB], { type: MT }) : CB);
...
O.preprocess = (Source) => {
  ...
  if (Setting.ReplaceRules)
    Source.Content = Setting.ReplaceRules.reduce(
      (SourceContent, Rule) => SourceContent.replace(Rule[0], Rule[1]),
      Source.Content
    );
  if (Setting.ResolveRules) {
    // RRR
    const FileDir = Source.Path.replace(/\/?[^\/]+$/, "");
    Setting.ResolveRules.forEach((ResolveRule) =>
      ResolveRule.Patterns.forEach((Pattern) => {
        ...
        Reses.forEach((Res) => {
          ...
          const ResPaths = O.getPath(
            FileDir,
            (!/^(\.*\/+|#)/.test(ResPathInSource) ? "./" : "") + ResPathInSource
          ).split("#");
          if (!ExtRE.test(ResPaths[0])) return;
          const Resource = O.src({ Path: ResPaths[0] });
          Resources.push(Resource);
          Promises.push(
            O.file(Resource, { Preprocess: true, URI: true }).then(
              (ChildSource) => {
                ResPaths[0] = ChildSource.URI;
                Source.Content = Source.Content.replace(
                  Res,
                  Res.replace(ResPathInSource, ResPaths.join("#"))
                );
              }
            )
          );
...
O.preprocess.getSetting = (FilePath) => {
  const PpSs = O.preprocess.Settings;
  for (const Ext in PpSs)
    if (new RegExp("\\.(" + Ext + ")$", "i").test(FilePath))
      return typeof PpSs[Ext].init == "function" ? PpSs[Ext].init() : PpSs[Ext];
  return null;
};
...
O.openDocument = (Source) =>
  O.file(Source).then((Source) =>
    new DOMParser().parseFromString(
      Source.Content,
      /\.(xml|opf|ncx)$/i.test(Source.Path) ? "text/xml" : "text/html"
    )
  );
...
O.editCSSRules = function () {
  ...
  if (typeof arguments[0] == "function")
    (Doc = arguments[1]), (fun = arguments[0]);
  else if (typeof arguments[1] == "function")
    (Doc = arguments[0]), (fun = arguments[1]);
  if (!Doc) Doc = document;
  if (!Doc.styleSheets || typeof fun != "function") return;
  sML.forEach(Doc.styleSheets)((StyleSheet) =>
...
O.editCSSRulesOfStyleSheet = (StyleSheet, fun) => {
  ...
  for (let l = StyleSheet.cssRules.length, i = 0; i < l; i++) {
    ...
    /**/ if (CSSRule.cssRules) O.editCSSRulesOfStyleSheet(CSSRule, fun);
      ...
      O.editCSSRulesOfStyleSheet(CSSRule.styleSheet, fun);
    else fun(CSSRule);
  }
};
...
O.getWritingMode = (Ele) => {
  ...
  if (!O.WritingModeProperty)
    return CS["direction"] == "rtl" ? "rl-tb" : "lr-tb";
  else if (/^vertical-/.test(CS[O.WritingModeProperty]))
    return (
      (CS["direction"] == "rtl" ? "bt" : "tb") +
      "-" +
      (/-lr$/.test(CS[O.WritingModeProperty]) ? "lr" : "rl")
    );
  else if (/^horizontal-/.test(CS[O.WritingModeProperty]))
    return (
      (CS["direction"] == "rtl" ? "rl" : "lr") +
      "-" +
      (/-bt$/.test(CS[O.WritingModeProperty]) ? "bt" : "tb")
    );
  else if (/^(lr|rl|tb|bt)-/.test(CS[O.WritingModeProperty]))
    return CS[O.WritingModeProperty];
};

O.getElementInnerText = (Ele) => {
  let InnerText = "InnerText";
  const Copy = document.createElement("div");
  Copy.innerHTML = Ele.innerHTML.replace(
    / (src(set)?|source|(xlink:)?href)=/g,
    " data-$1="
  );
  sML.forEach(Copy.querySelectorAll("svg"))((Ele) =>
    Ele.parentNode.removeChild(Ele)
  );
  sML.forEach(Copy.querySelectorAll("video"))((Ele) =>
    Ele.parentNode.removeChild(Ele)
  );
  sML.forEach(Copy.querySelectorAll("audio"))((Ele) =>
    Ele.parentNode.removeChild(Ele)
  );
  sML.forEach(Copy.querySelectorAll("img"))((Ele) =>
    Ele.parentNode.removeChild(Ele)
  );
  sML.forEach(Copy.querySelectorAll("script"))((Ele) =>
    Ele.parentNode.removeChild(Ele)
  );
  sML.forEach(Copy.querySelectorAll("style"))((Ele) =>
    Ele.parentNode.removeChild(Ele)
  );
...
O.getElementCoord = (Ele, OPa) => {
  ...
  while (Ele.offsetParent != OPa)
    (Ele = Ele.offsetParent),
      (Coord.X += Ele.offsetLeft),
      (Coord.Y += Ele.offsetTop);
  return Coord;
};
...
O.getPath = function () {
  let Origin = "",
    Path = arguments[0];
  if (arguments.length == 2 && /^[\w\d]+:\/\//.test(arguments[1]))
    Path = arguments[1];
  else
    for (let l = arguments.length, i = 1; i < l; i++)
      Path += "/" + arguments[i];
  Path.replace(/^([a-zA-Z]+:\/\/[^\/]+)?\/*(.*)$/, (M, P1, P2) => {
    (Origin = P1), (Path = P2);
  });
  while (/([^:\/])\/{2,}/.test(Path))
...
O.getViewportByImage = (Img) => {
  if (Img && /^img$/i.test(Img.tagName)) {
    ...
    return {
      ...
      Height: parseInt(ImageStyle.height),
    };
  }
  return null;
};
...
O.stopPropagation = (Eve) => {
  Eve.stopPropagation();
  return false;
};

O.preventDefault = (Eve) => {
  Eve.preventDefault();
  return false;
};
...
O.getCoordInViewport = (Coord, Doc) => {
  if (Doc == document) {
    ...
    const Main = R.Main;
    const MainTransformation = I.Loupe.CurrentTransformation || {
      Scale: 1,
      TranslateX: 0,
      TranslateY: 0,
    };
    ...
    if (Item["rendition:layout"] != "pre-paginated" && !Item.Outsourcing)
      (ItemCoordInMain.X += S["item-padding-left"]),
        (ItemCoordInMain.Y += S["item-padding-top"]);
    Coord.X = Math.floor(
      Main.offsetLeft +
        (MainTransformOriginX_InMain +
          MainTranslationX +
          (ItemCoordInMain.X +
            Coord.X * ItemScale -
            Main.scrollLeft -
...
O.Biscuits = {
  ...
  initialize: (Tag) => {
    ...
    if (typeof Tag != "string") {
      O.Biscuits.LabelBase =
        "BibiBiscuits:" +
        P.Script.src.replace(
          new RegExp("^" + O.Origin.replace(/([\/\.])/g, "\\$1")),
          ""
        );
    ...
    const Label = (O.Biscuits.Labels[Tag] =
      O.Biscuits.LabelBase + (Tag == "Book" ? "#" + B.ID : ""));
    const BiscuitsOfTheLabel = localStorage.getItem(Label);
    O.Biscuits.Memories[Label] = BiscuitsOfTheLabel
      ? JSON.parse(BiscuitsOfTheLabel)
      : {};
    return O.Biscuits.Memories[Label];
  },
  remember: (Tag, Key) => {
    if (!Tag || typeof Tag != "string" || !O.Biscuits.Labels[Tag])
      return O.Biscuits.Memories;
    const Label = O.Biscuits.Labels[Tag];
    return !Key || typeof Key != "string"
      ? O.Biscuits.Memories[Label]
      : O.Biscuits.Memories[Label][Key];
  },
  memorize: (Tag, KnV) => {
    if (!Tag || typeof Tag != "string" || !O.Biscuits.Labels[Tag]) return false;
    const Label = O.Biscuits.Labels[Tag];
    if (KnV && typeof KnV == "object")
      for (const Key in KnV) {
        const Val = KnV[Key];
        try {
          if (
            Val &&
            typeof Val != "function" &&
            typeof JSON.parse(JSON.stringify({ [Key]: Val }))[Key] !=
              "undefined"
          )
            O.Biscuits.Memories[Label][Key] = Val;
    ...
    return localStorage.setItem(
      Label,
      JSON.stringify(O.Biscuits.Memories[Label])
    );
  },
  forget: (Tag, Keys) => {
    if (!Tag) {
    ...
    [].forEach.call(iframes, function(frame) {
        var iframeSelection = frame.contentWindow.getSelection(); 
          if (iframeSelection.toString().length > 0) {
            //    alert(iframeSelection);
            //    console.log(iframeSelection.toString());
...
E.initialize = () => {
  ...
  E.add = function (/*[Tar,]*/ Nam, fun, Opt) {
    if (Array.isArray(arguments[0]))
      return arguments[0].forEach((AI) =>
        E.add(AI, arguments[1], arguments[2], arguments[3])
      );
    if (Array.isArray(arguments[1]))
      return arguments[1].forEach((AI) =>
        E.add(arguments[0], AI, arguments[2], arguments[3])
      );
    if (Array.isArray(arguments[2]) && typeof arguments[2][0] == "function")
      return arguments[2].forEach((AI) =>
        E.add(arguments[0], arguments[1], AI, arguments[3])
      );
    let Tar = document;
    if (typeof fun != "function")
      (Tar = arguments[0]),
        ...
        (fun = arguments[2]),
        (Opt = arguments[3]);
    return /^bibi:/.test(Nam)
      ? E.CustomEvents.add(Tar, Nam, fun)
      : Tar.addEventListener(Nam, fun, Opt);
  };
  E.remove = function (/*[Tar,]*/ Nam, fun, Opt) {
    if (Array.isArray(arguments[0]))
      return arguments[0].forEach((AI) =>
        E.remove(AI, arguments[1], arguments[2], arguments[3])
      );
    if (Array.isArray(arguments[1]))
      return arguments[1].forEach((AI) =>
        E.remove(arguments[0], AI, arguments[2], arguments[3])
      );
    if (Array.isArray(arguments[2]) && typeof arguments[2][0] == "function")
      return arguments[2].forEach((AI) =>
        E.remove(arguments[0], arguments[1], AI, arguments[3])
      );
    let Tar = document;
    if (typeof fun != "function")
      (Tar = arguments[0]),
        ...
        (Opt = arguments[3]);
    return /^bibi:/.test(Nam)
      ? E.CustomEvents.remove(Tar, Nam, fun)
      : Tar.removeEventListener(Nam, fun, Opt);
  };
  E.bind = function () {
    return E.CustomEvents.bind.apply(E.CustomEvents, arguments);
  };
  E.unbind = function () {
    return E.CustomEvents.unbind.apply(E.CustomEvents, arguments);
  };
  E.dispatch = function () {
    return E.CustomEvents.dispatch.apply(E.CustomEvents, arguments);
  };
  delete E.initialize;
};
...
M.judge = (Msg, Origin) =>
  ...
  S["trustworthy-origins"].includes(Origin);

M.post = (Msg) =>
  !M.judge(Msg, O.ParentOrigin)
    ? false
    : window.parent.postMessage(Msg, window.parent.location.origin);

M.receive = (Eve) => {
  if (!Eve || !M.judge(Eve.data, Eve.origin)) return false;
  try {
    const Data = JSON.parse(Eve.data);
    if (!Data || typeof Data != "object") return false;
    for (const EventName in Data)
      if (/^bibi:commands:/.test(EventName))
        E.dispatch(EventName, Data[EventName]);
    return true;
  } catch (Err) {}
  return false;
};
...
X.load = (Xtn) =>
  new Promise((resolve, reject) => {
    if (!Xtn["src"] || typeof Xtn["src"] != "string")
      return reject(
        `"path" of the Extension Seems to Be Invalid. ("${Xtn["src"]}")`
      );
    const XO = new URL(Xtn["src"]).origin;
    if (!S["trustworthy-origins"].includes(XO))
      return reject(`The Origin Is Not Allowed. ("${Xtn["src"]}")`);
    Xtn.Script = document.head.appendChild(
      sML.create("script", {
...
X.add = (XMeta) => {
  ...
  if (typeof XMeta["id"] == "undefined")
    return XScript.reject(`"id" of the extension is undefined.`);
  if (typeof XMeta["id"] != "string")
    return XScript.reject(`"id" of the extension is invalid.`);
  if (!XMeta["id"]) return XScript.reject(`"id" of the extension is blank.`);
  if (X[XMeta["id"]])
    return XScript.reject(
      `"id" of the extension is reserved or already used by another. ("${XMeta["id"]}")`
    );
  ...
  return function (onR) {
    if (Xtn && typeof onR == "function")
      E.bind("bibi:readied", () => onR.call(Xtn, Xtn));
    return function (onP) {
      if (Xtn && typeof onP == "function")
        E.bind("bibi:prepared", () => onP.call(Xtn, Xtn));
      return function (onO) {
        if (Xtn && typeof onO == "function")
          E.bind("bibi:opened", () => onO.call(Xtn, Xtn));
      };
    };
  };
};
